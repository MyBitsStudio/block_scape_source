...
public final class Index {
	...
	Index(@NotNull MainFile index255, MainFile mainFile, int[] keys) {
	...
	public void putArchive(int id, int archiveId, int newArchiveId, Store originalStore, boolean rewriteTable, boolean resetCache) {
		try {
			Index originalIndex = originalStore.getIndexes()[id];
	...
	public void putFile(int archiveId, int fileId, byte[] data) {
		putFile(archiveId, fileId, Constants.GZIP_COMPRESSION, data, null, true, true, -1, -1);
	}
	...
	public void putFile(int archiveId, int fileId, int compression, byte[] data, int[] keys, boolean rewriteTable, boolean resetCache, int archiveName, int fileName) {
		if (archiveExists(archiveId)) {
			cacheArchiveFiles(archiveId, keys);
		} else {
			table.addEmptyArchiveReference(archiveId);
