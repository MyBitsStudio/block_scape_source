...
public final class ReferenceTable {
	...
	public void sortTable() {
		...
		for (int validArchiveId : validArchiveIds) {
			ArchiveReference archive = archives[validArchiveId];
	...
	public Object[] encodeHeader(MainFile mainFile) {
		...
		if (named) {
			for (int validArchiveId : validArchiveIds) {
				stream.writeInt(archives[validArchiveId].getNameHash());
			}
		}
		if (usesWhirpool) {
			for (int validArchiveId : validArchiveIds) {
				stream.writeBytes(archives[validArchiveId].getWhirpool());
			}
		}
		for (int validArchiveId : validArchiveIds) {
			stream.writeInt(archives[validArchiveId].getCRC());
		}
		for (int validArchiveId : validArchiveIds) {
			stream.writeInt(archives[validArchiveId].getRevision());
		}
		for (int validArchiveId : validArchiveIds) {
			int value = archives[validArchiveId].getValidFileIds().length;
			if (protocol >= 7) {
