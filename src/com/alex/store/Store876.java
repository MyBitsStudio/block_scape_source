...
public final class Store876 {
	...
	public Store876(String path) throws IOException {
		this(path, true);
	}

	public Store876(String path, boolean newProtocol) throws IOException {
		this(path, newProtocol, null);
	}

	public Store876(String path, boolean newProtocol, int[][] keys) throws IOException {
		this.path = path;
	...
	public byte[] generateIndex255Archive255Outdated() {
		...
		for (Index value : indexes) {
			...
			stream.writeInt(value.getCRC());
			stream.writeInt(value.getTable().getRevision());
		}
		byte[] archive = new byte[stream.getOffset()];
		stream.setOffset(0);
		stream.getBytes(archive, 0, archive.length);
	 ...
	 * returns index
	 */
	public void addIndex(boolean named, boolean usesWhirpool, int tableCompression) throws IOException {
	...
	public void resetIndex(int id, @NotNull Index[] indexes, boolean named, boolean usesWhirpool, int tableCompression) throws IOException {
