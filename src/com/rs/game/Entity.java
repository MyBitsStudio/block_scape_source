...
public abstract class Entity extends WorldTile {
	...
	public boolean restoreHitPoints() {
		...
		if (hitpoints > maxHp) {
			...
			if (this instanceof Player) {
				Player player = (Player) this;
				if (player.getPerks().isUnlocked(Perks.HAZELMERE.getId()) && hitpoints < maxHp) {
					setHitpoints(hitpoints + 10);
				}
				if (player.getPrayer().usingPrayer(0, 9) && !player.getPerks().isUnlocked(Perks.HAZELMERE.getId()) && hitpoints < maxHp) {
					setHitpoints(hitpoints + 1);
