...
public class BountyHunter implements Serializable {
	...
	private static final int[] revDrops = new int[] { 13870, 13873, 13876, 13879, 13880, 13881, 13882, 13883, 13944,
	...
	public static void reward2(WorldTile tile, int percentage) {
		...
		if (Utils.getRandom(chance) >= 50) {// 1/4 chance at most
			...
			System.out.println("Dropping " + ItemDefinitions.getItemDefinitions(id).getName() + ", which is worth "
	...
	/**
	 * Rewards 120 is the maximum percentage for bounty targets, otherwise it's 100
	 */
	public void reward(WorldTile tile, int percentage) {
		...
		chance /= 3.2;
		if (Utils.getRandom(chance) >= 50) {// 1/4 chance at most
			World.addGroundItem(new Item(numberOneRewards[Utils.getRandom(numberOneRewards.length - 1)]), tile, player,
		...
		if (Utils.getRandom(chance) >= 60) {
			World.addGroundItem(new Item(numberTwoRewards[Utils.getRandom(numberTwoRewards.length - 1)]), tile, player,
					true, 600000);// 10 Minutes
			chance -= 10;
		}
		if (Utils.getRandom(chance) >= 50) {
			World.addGroundItem(new Item(numberThreeRewards[Utils.getRandom(numberThreeRewards.length - 1)]), tile,
					player, true, 600000);// 10 Minutes
			chance -= 10;
		}
		if (Utils.getRandom(chance) >= 40) {
			World.addGroundItem(new Item(numberFourRewards[Utils.getRandom(numberFourRewards.length - 1)]), tile,
					player, true, 600000);
			chance -= 10;
		}
	...
	public void sendInterface() {
		// player.getInterfaceManager().sendProperOverlay(591);
