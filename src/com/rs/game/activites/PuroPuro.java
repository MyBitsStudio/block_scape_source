...
public class PuroPuro extends Controller {
	...
	public static void initPuroImplings() {
		for (int i = 0; i < 5; i++) {
			for (int index = 0; index < FlyingEntities.values().length; index++) {
				if (i > 2)
					continue;
				World.spawnNPC(FlyingEntities.values()[index].getNpcId(), 
					new WorldTile(Utils.random(2558 + 3, 2626 - 3), Utils.random(4285 + 3, 4354 - 3), 0), -1, false);
			}
		}
	}
	...
	public static void confirmPuroSelection(Player player) {
		...
		if (player.getInventory().addItemDrop(exchangedItem.getId(), exchangedItem.getAmount())) {
			...
			player.sendMessage("You exchange the required items for: " 
