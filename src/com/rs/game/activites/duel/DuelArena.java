...
import com.rs.game.player.content.corrupt.futures.impl.DuelArenaFuture;
...
public class DuelArena extends Controller {
	...
	private void beginBattle(boolean started) {
		...
		Item[] stake = new Item[28], stake2 = new Item[28];
		int value = 0, value2 = 0;
		for(int i = 0; i < player.getDuelRules().getStake().getItems().length; i++) {
			Item item = player.getDuelRules().getStake().getItems()[i];
			if(item == null)
				continue;
			if(item.getId() == 995){
		...
		for(int i = 0; i < target.getDuelRules().getStake().getItems().length; i++){
			Item item = target.getDuelRules().getStake().getItems()[i];
			if(item == null)
				continue;
			if(item.getId() == 995){
				value2 = item.getAmount();
				continue;
			}
			stake2[i] = item;
		}

		DuelArenaFuture future = new DuelArenaFuture(player, target)
				.addBalance(BlockchainUtils.coinToChain(value), BlockchainUtils.coinToChain(value2))
				.addItems(List.of(stake), List.of(stake2))
				.build();
		future.process();

//		DuelArenaFuture future = new DuelArenaFuture()
//				.startFuture(player, target)
//				.setValues("value", value)
//				.setValues("value2", value2)
//				.setItems("stake", stake)
//				.setItems("stake2", stake2)
//				.setProperty("player1", player.getDisplayName())
//				.setProperty("player1_wallet", player.getWallet().getPublicAddress())
//				.setProperty("player2", target.getDisplayName())
//				.setProperty("player2_wallet", target.getWallet().getPublicAddress())
//				.setSignatures();
//		future.addToFutures(player, target, 30, "duel_arena");
	...
	public void closeDuelInteraction(DuelStage stage) {
		synchronized (this) {
			...
			if (!(controler instanceof DuelArena targetConfiguration))
				return;
		...
		if (duelStage == DuelStage.DONE) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					} else player.getControlerManager().startControler("DuelControler");
				}
			});
			player.getInventory().getItems().addAll(player.getDuelRules().getStake());
			player.getInventory().init();
	...
	public void endDuel(final Player victor, final Player loser, boolean isDraw) {
		...
		if (rules != null && !rules.hasRewardGiven()) {
			assert victor != null;
//			DuelArenaFuture future = (DuelArenaFuture) player.getWallet().getFuture("duel_arena");
//			future.onComplete(victor, isDraw);
...
//			}
		...
		loser.sendMessage(isDraw ? "The battle has ended in a draw."
				: "Oh dear, it seems you have lost to " + Objects.requireNonNull(victor).getDisplayName() + ".");
		assert victor != null;
	...
	private @NotNull String getAcceptMessage(boolean firstStage) {
	...
	private WorldTile @NotNull [] getPossibleWorldTiles() {
	...
	private WorldTile @NotNull [] getPossibleWorldTilesSummoning() {
	...
	public boolean isWearingTwoHandedWeapon() {
		...
		return weaponId == 4153 || weaponId == 11235 || weaponId == 861 || weaponId == 18353 || weaponId == 20171;
	}
	...
	private void openConfirmationScreen() {
		player.getInterfaceManager().sendInterface(626);
		refreshScreenMessage(false, false);
	}
	...
	private void setRules(@NotNull DuelRules rules, int componentId, int slotId) {
	...
	@Override
	public boolean processObjectClick1(@NotNull WorldObject object) {
	...
	private void refreshItems(Item @NotNull [] itemsBefore) {
	...
	private void removeEquipment() {
		...
		for (int i = 10; i < 23; i++) {
			if (i == 14) {
				if (player.getEquipment().hasTwoHandedWeapon() || isWearingTwoHandedWeapon())
	...
	private void sendFinishInterface(@NotNull Player player, @NotNull Player loser) {
	...
	private void sendOptions(@NotNull Player player) {
