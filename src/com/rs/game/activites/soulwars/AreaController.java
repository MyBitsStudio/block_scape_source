				...
				case WorldPacketsDecoder.ACTION_BUTTON2_PACKET:
					if (checkZeals(2, false)) {
						if (!player.getInventory().hasFreeSlots()) {
							player.sendMessage("You don't have any space to obtain your special reward!");
							return false;
						}
						checkZeals(2, true);
...
//						if (reward.getId() == 995) {
//							SpawnTransaction spawn = chain.getContract(chain.getProperty("Block Coin")).getGenesis().sendSpawnTransaction(player.getWallet().getPublicKey(),
//									ChainHelper.microsToCoin(reward.getAmount()), player);
//							if(spawn != null)
//								chain.addTransactionToChain(spawn);
//						} else {
//							SpawnWithoutCoinTransaction spawn = chain.getContract(chain.getProperty("Block Coin")).getGenesis().sendSpawnWithoutCoinTransaction(player.getWallet().getPublicKey(),
//									 reward);
//							if(spawn != null)
//								chain.addTransactionToChain(spawn);
//						}
	...
	private void exchangeZealsForXp(int componentId, final int zeals) {
		if (checkZeals(zeals, false) /*player.getGameModeManager().getMode(0) != 2*/) {
	...
	private boolean checkZeals(double zeals, boolean remove) {
		//MasterChain chain = MasterChain.singleton;
//		if (player.getWallet().getBalance(chain.getContract(chain.getProperty("Zeal")).getAddress()) < zeals) {
//			player.sendMessage("You don't have enough zeals to spend.");
//			return false;
//		}
//		if (remove) {
//			TransferTransaction trans = player.getWallet().sendCustomCoin(chain.getContract(chain.getProperty("Zeal")).getGenesis().getPublicKey(),
//					chain.getContract(chain.getProperty("Zeal")).getAddress(), zeals);
//			if(trans != null){
//				chain.addTransactionToChain(trans);
//			}
//			player.getPackets().sendConfigByFile(5827, player.getZeals());
//		}
		return true;
