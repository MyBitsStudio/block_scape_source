...
public final class SoulWarsManager {
	...
	private final HashMap<PlayerType, TimerTask> tasks = new HashMap<>(
	...
	private void sendPlayers(boolean create) {
		...
		int size;
		if (create) {
			size = Math.min(blue.size(), red.size());
			if (size < 1)
				return;
			for (int i = 0; i < 2; i++) {
				int index = 0;
			...
			final int lobbySize = Math.min(lobbyBlue.size(), lobbyRed.size());
			final int[] takeOutEachTeam = new int[2];
			takeOutEachTeam[0] = lobbySize + (gameBlue.size() > gameRed.size() ? Math.min(size + lobbySize, lobbyRed.size()) : 0);
			takeOutEachTeam[1] = lobbySize + (gameRed.size() > gameBlue.size() ? Math.min(size + lobbySize, lobbyBlue.size()) : 0);
