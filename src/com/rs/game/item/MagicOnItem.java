...
//				player.getSkills().addXp(Skills.MAGIC, 90);
	...
	public static void processAlchemy(Player player, Item item, boolean low) {
		...
		if (player.getActionManager().getActionDelay() != 0)
			return;
		if (Magic.checkSpellRequirements(player, (low ? 21 : 55), true, 554, (low ? 3 : 5), 561, 1)) {
			if (!ItemConstants.isTradeable(item)) {
				player.sendMessage("You can't " + (low ? "low" : "high") + " alch this!");
				return;
			}
			...
			player.getActionManager().setActionDelay(low ? 3 : 5);

//			MasterChain chain = MasterChain.singleton;
//			BurnTransaction burn = player.getWallet().sendBurn(chain.getContract(chain.getProperty("Block Coin")).getGenesis().getPublicKey(), 0.000000,
//					chain.getContract(chain.getProperty("Block Coin")), player, item);
//			SystemTransaction trans = chain.getContract(chain.getProperty("Block Coin")).getGenesis().sendSystemTransaction(player.getWallet().getPublicKey(),
//					ChainHelper.microsToCoin(EconomyPrices.getAlchPrice(item, low)));
//			if(burn != null && trans != null){
//				chain.addTransactionToChain(burn);
//				chain.addTransactionToChain(trans);
//			}
			player.getInventory().deleteItem(item.getId(), 1);
//			if(player.getGameModeManager().getMode(0) == 2)
//				player.getPrestige().handleTask(Prestige.MAGIC, -1, 1, "Alchemy");
//			else
				player.getSkills().addXp(Skills.MAGIC, (low ? 125 : 225));
