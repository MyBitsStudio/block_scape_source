...
public abstract class BossInstance {
	...
	public void enterInstance(Player player, boolean login) {
		synchronized (BossInstanceHandler.LOCK) {
			...
			if (!isPublic())
				player.getPackets()
						.sendGameMessage("Welcome to this session against <col=00FFFF><shad=000000>" + getInstanceName()
								+ "</shad></col>. This arena will expire in " + (settings.getTimeRemaining() / 60000)
								+ " minutes.");
			if (!login)
