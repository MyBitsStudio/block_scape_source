...
import com.rs.game.world.controllers.AraxyteHyveController;
     ...
     */
 	...
 	private void sendObject(Player player) {
		...
		OwnedObjectManager.addOwnedObjectManager(player, new WorldObject[]
				...
				{ 30000 });
		player.getAdventurerLogs().addToBossKills(this.id);
		player.sendMessage("You've killed a total of "+Colors.red+player.getAdventurerLogs().getBossKills().get(this.id)
		...
		for (Player all : AraxyteHyveController.getPlayers()) {
			...
			all.sendMessage(Colors.darkRed+player.getDisplayName()+" has slain the Araxxor; "
					+ "his total Araxxor kills: "+player.getAdventurerLogs().getBossKills().get(this.id)+".");
		}
		if (rewards != null) {
