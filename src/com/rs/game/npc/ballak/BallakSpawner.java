...
public enum BallakSpawner {
	...
	public static void scheduleSpawn() {
		CoresManager.slowExecutor.scheduleWithFixedDelay(new Runnable() {
			...
			NPC npc;
			String[] messages = { 
				"Will no warrior come forth to defeat Bal'Lak ",
				"Bal'lak still stands strong! ",
				"Has no one the strength to defeat Bal'Lak ",
				"Who will step forth to defeat Bal'lak? ",
			};
    	    ...
    	    public void run() {
    	    	...
    	    	if (npc != null && !npc.hasFinished()) {
    	    		if (npc.isUnderCombat()) {
    	    			System.out.println("<span style='#00FF00'>Unable to spawn Bal'lak. Currently under combat.</span>");
    	    			return;
    	    		}
    	    		World.sendWorldMessage("<col=FF0000>"+messages[Utils.random(messages.length -1)] + spawn.getLocation()+". ", false);
    	    		return;
    	    	}
				...
				World.sendWorldMessage("--- <col=FF0000>Notice: Bal'Lak has appeared " + spawn.getLocation() + "!",
						false);
    	    	System.out.println("<span style='color:green;'>Bal'lak has been spawned "+spawn.getLocation()+"</span>");
    	    }
