	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utils.getRandom(2) == 0) { // range attack
			...
			for (Entity t : npc.getPossibleTargets()) {
				delayHit(npc, 1, t, getRangeHit(npc, getRandomMaxHit(npc, 300, NPCCombatDefinitions.RANGE, t)));
				World.sendProjectile(npc, t, 1200, 41, 16, 41, 35, 16, 0);
			}
		} else { // melee attack
			if (npc.withinDistance((Entity) target, 2)) {
				npc.setNextAnimation(new Animation(defs.getAttackEmote()));
				delayHit(npc, 0, target,
						getMeleeHit(npc, getRandomMaxHit(npc, 450, NPCCombatDefinitions.MELEE, target)));
