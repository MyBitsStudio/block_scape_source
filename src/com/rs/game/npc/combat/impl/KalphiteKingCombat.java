...
public class KalphiteKingCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (npc.getId() == 16699) { //Ranged KK
		    ...
		    case 3:
				...
				break;
		    case 4:
		    	meleePush(npc, target);//meleeBleed(npc, target);
				break;
		    case 5:
		    	meleeStomp(npc, target);
				rush(npc, target);
		    ...
		    case 2:
				...
				break;
		    case 3:
		    	meleePush(npc, target);//mageBall(npc, target, 0);
				break;
		    case 4:
				dig(npc, target);
				break;
    ...
    private void rangeBasic(NPC npc, Entity target) {
		...
		//for (Entity e : npc.getPossibleTargets()) {
		    //Hit hit = getRangeHit(npc, getRandomMaxHit(npc, 550, NPCCombatDefinitions.RANGE, e));
	...
	private void rangeIncendiaryShot(NPC npc, Entity target) {
		...
		while (c < 3) {
		    for (final Entity t : list) {
		    	t.applyHit(new Hit(npc, Utils.random(50), HitLook.RANGE_DAMAGE, 10));
		    	//Hit hit = new Hit(npc, 50 + Utils.random(50), HitLook.RANGE_DAMAGE);
		...
		if (target instanceof Player) {
			if (Utils.isOnRange(npc, target, 5)) {
			  ...
			  //target.applyHit(new Hit(npc, 700, HitLook.MELEE_DAMAGE));
				...
				target.getHitpoints() > 20 ? (int) (target.getHitpoints() * 0.65) : 1,
