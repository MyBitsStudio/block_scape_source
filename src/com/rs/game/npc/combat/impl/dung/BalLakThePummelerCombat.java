...
public class BalLakThePummelerCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (Utils.random(5) == 0) {
			...
			for (Entity t : boss.getPossibleTargets()) {
				...
				if (t instanceof Player) {
					...
					if ((damage > 0 || damage2 > 0) && player.getPrayerDelay() < Utils.currentTimeMillis()) {
						player.setPrayerDelay(Utils.currentTimeMillis() + 8000);
						player.getPackets()
		...
		case 0:// reg melee left
			...
			delayHit(npc, 0, target, getMeleeHit(npc, getMaxHit(npc,
					(int) (npc.getMaxHit(NPCCombatDefinitions.MELEE) * 0.8), NPCCombatDefinitions.MELEE, target)));
			delayHit(npc, 2, target, getMeleeHit(npc, getMaxHit(npc,
					(int) (npc.getMaxHit(NPCCombatDefinitions.MELEE) * 0.8), NPCCombatDefinitions.MELEE, target)));
			WorldTasksManager.schedule(new WorldTask() {
		...
		case 1:// magic attack multi
			...
			for (Entity t : npc.getPossibleTargets()) {
				...
				delayHit(npc, 1, t,
						getMagicHit(npc, getMaxHit(npc, (int) (boss.getMaxHit() * 0.6), NPCCombatDefinitions.MAGE, t)));
			}
