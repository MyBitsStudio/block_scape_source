	...
	public int attack(NPC npc, Entity target) {
		...
		if (atDistance || Utils.random(3) == 0) {
			...
			if (rangeAttack) {
				if (manager.getParty().getTeam().size() > 1 || Utils.random(3) == 0) {
						...
						public void run() {
							...
							if (cycles == 1) {
								...
								for (WorldTile tile : knifeTargets) {
									...
									World.sendProjectile(boss, boss.getNextPath(), tile, 2853, 18, 18, 50, 50, 0, 0);
									int delay = 3;
									entityLoop: for (Entity t : boss.getPossibleTargets()) {
										if (!t.matches(tile))
											continue entityLoop;
										delayHit(boss, delay, t, getRangeHit(boss,
												getMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitions.RANGE, t)));
									}
								}
								stop();
								return;
					...
					delayHit(boss, 1, target,
							getRangeHit(boss, getMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitions.RANGE, target)));
				}
			} else {
				if (Utils.random(7) == 0) {
				...
				int damage = getMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitions.MAGE, target);
			...
			delayHit(boss, 0, target,
					getMeleeHit(boss, getMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitions.MELEE, target)));
			return 4;
		}
	}
}
