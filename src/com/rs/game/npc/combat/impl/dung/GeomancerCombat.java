...
public class GeomancerCombat extends CombatScript {
	...
	private void sendEarthBlast(NPC npc, Entity target, boolean multiAttack) {
		...
		for (Entity t : npc.getPossibleTargets()) {
			...
			delayHit(npc, 1, t, getMagicHit(npc, getMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitions.MAGE) * .7),
	...
	private void sendEntangle(NPC npc, Entity target) {
		...
		int damage = getMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitions.MAGE) * 0.95), NPCCombatDefinitions.MAGE,
				target);

		if (damage > 0) {
			target.setNextGraphics(new Graphics(180, 75, 100));
			if (!target.isBoundImmune()) {
				target.setBoundDelay(20);
				target.setBoundImmune(Utils.currentTimeMillis() + 7000);
			}
			...
			if (damage > 0) {
				if (t instanceof Player) {
					...
					if (player.getPrayer().hasPrayersOn()) {
						...
						player.setPrayerDelay(Utils.currentTimeMillis() + 25000);
