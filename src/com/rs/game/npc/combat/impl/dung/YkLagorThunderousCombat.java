		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (disablePrayers) {
					...
					player.setPrayerDelay(Utils.currentTimeMillis() + (8 + Utils.random(17)) * 1000);
				}
			}
		}, 1);
	}
	...
	public static void sendMagicalAttack(YkLagorThunderous npc, boolean specialAttack) {
		...
		if (npc.getPossibleTargets().size() > 0) {
			for (Player player : npc.getManager().getParty().getTeam()) {
				...
				delayHit(npc, 1, player, getMagicHit(npc, getMaxHit(npc, NPCCombatDefinitions.MAGE, player)));
