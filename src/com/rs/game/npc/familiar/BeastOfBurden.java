...
public class BeastOfBurden implements Serializable {
	...
	public void dropBob() {
		if (familiar.getOwner().getControlerManager().getControler() != null/*
				&& (familiar.getOwner().getControlerManager().getControler() instanceof InstancedPVPControler)*/) {
			beastItems.reset();
			return;
		}
	...
	public void addItem(int slot, int amount) {
		...
		if (item == null)
			return;
		if (canDepositOnly) {
			player.sendMessage("You cannot store items in this familiar.");
			return;
		} else if (!ItemConstants.isTradeable(item) && item.getId() != 23194 || item.getId() == 4049
