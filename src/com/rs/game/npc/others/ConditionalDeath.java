...
public class ConditionalDeath extends NPC {
	...
	private boolean removeItem(Player player) {
		if (this.getHitpoints() < (getMaxHitpoints() * 0.1) && (player.getEquipment().getWeaponId() == requiredItem
	...
	public void sendDeath(Entity source) {
		if (source instanceof Player) {
			...
			if ((player.getSlayerManager().hasLearnedQuickBlows() || player.getEquipment().getWeaponId() == requiredItem
					|| player.getEquipment().getGlovesId() == requiredItem) && useHammer(player))
				return;
