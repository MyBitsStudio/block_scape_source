...
public class AgilityPenguinNPC extends NPC {
     ...
     */
    public void giveReward(final @NotNull Player player) {
		...
		Item[] toAdd = new Item[2];
    	if (player.getLapsRan() >= 250) {
			if (!player.hasItem(new Item(14936))) {
				toAdd[0] = new Item(14936, 1);
			}
		}
    	if (player.getLapsRan() >= 500) {
			if (!player.hasItem(new Item(14938))) {
				toAdd[1] = new Item(14938, 1);
			}
		}
		stop = true;
//		MasterChain chain = MasterChain.singleton;
//		SpawnTransaction transaction = chain.getContract(chain.getProperty("Block Coin")).getGenesis().sendSpawnTransaction(player.getWallet().getPublicKey(),
//				ChainHelper.microsToCoin(Utils.random(250000)), player, toAdd);
//		if(transaction != null){
...
//			chain.addTransactionToChain(transaction);
//		}
		player.lock(3);
