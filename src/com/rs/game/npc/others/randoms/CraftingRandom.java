...
public class CraftingRandom extends NPC {

	
	/**
	 * 
	 */
	private static final long serialVersionUID = 2536061290736425102L;
	
	private Player target;
    private long createTime;
    private boolean stop;

    public CraftingRandom(WorldTile tile, Player target) {
    	super(7954, tile, -1, true, true);
    	this.target = target;
    ...
    public void giveReward(final Player player) {
    	...
    	if(player.getItemsMade() >= 25000) {
    		if (!player.hasItem(new Item(32777)) && !player.hasItem(new Item(32281)))
    			player.addItem(new Item(32777, 1));
    	}
		stop = true;
