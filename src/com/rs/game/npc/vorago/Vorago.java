...
public class Vorago extends NPC {// 89150
	...
	private final WorldTile[] locations = { new WorldTile(3095, 6120, 0), new WorldTile(3104, 6120, 0), new WorldTile(3112, 6120, 0), new WorldTile(3095, 6111, 0), new WorldTile(3104, 6111, 0), new WorldTile(3112, 6111, 0), new WorldTile(3095, 6103, 0), new WorldTile(3104, 6103, 0), new WorldTile(3112, 6103, 0) };
	...
	public Vorago(int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea, VoragoInstance instance) {
		...
		voragoPushRatio = hardMode ? (startingSize <= 6 ? voragoPushRatio : (voragoPushRatio + (0.03 * (startingSize - 6)))) : (startingSize <= 4 ? voragoPushRatio : (voragoPushRatio + (0.02 * (startingSize - 4))));
	}

	@Override
	public boolean canWalkNPC(int toX, int toY) {
		boolean hardMode = instance.getSettings().isHardMode();
		return !(getTemporaryAttributtes().get("BringHimDownClick") != null | (!hardMode && phase == 5) || (hardMode && phase >= 10)) && !(getTemporaryAttributtes().get("CantBeAttacked") != null || getTemporaryAttributtes().get("TheEnd") != null) && super.canWalkNPC(toX, toY);
	}
					...
					if (damage > 0) {// push players
						...
						for (Player player : instance.getPlayersOnBattle()) {
							...
							if (Utils.colides(new WorldTile(nextX, getY(), getPlane()), player, getSize(), player.getSize())) {
								if (nextX == ((hardMode && phase == 11) ? (voragoSpawnLocation.getX() + 12) : (voragoSpawnLocation.getX() - 10))) {
	...
	public void sendRealDeath(Player player) {
		...
		for (Player players : instance.getPlayersOnBattle()) {
			...
			players.resetCombat();
			players.resetWalkSteps();
			WorldTile sendTile = new WorldTile((hardMode && phase == 11) ? (getX() + 1) : (getX() - 1), getY(), getPlane());
			players.setNextWorldTile(new WorldTile((hardMode && phase == 11) ? (sendTile.getX() + Utils.random(5)) : (sendTile.getX() - Utils.random(5)), sendTile.getY(), sendTile.getPlane()));
		}
		player.lock();
		player.resetCombat();
				...
				public void run() {
					if (loop == 0) {
						...
						for (Player players : instance.getPlayersOnBattle()) {
							if (players == null || players.isDead())
								continue;
							Dialogue.sendNPCDialogueNoContinue(player, getId(), Dialogue.ANGRY, "Enough!", "If you truly are the Defeater, Roon-show me!");
						}
					} else if (loop == 6) {
						for (Player players : instance.getPlayersOnBattle()) {
							if (players == null || players.isDead() || players == player)
					...
					if (loop == 0) {
						...
						for (Player players : instance.getPlayersOnBattle()) {
							if (players == null || players.isDead())
								continue;
							players.unlock();
							player.getAdventurerLogs().addToBossKills(hardMode ? Vorago.super.id : -1); // TO FIX
							//players.setKillStats(hardMode ? 112 : 93, players.getKillStatistics(hardMode ? 112 : 93) + 1);
						}
						stop();
					}
					loop++;
	...
	private void sendDrops() {
		...
		for (int i = 0; i < dropsCount; i++) {
			...
			for (Drop drop : drops) {
				...
				if ((Utils.getRandomDouble(99) + 1) <= dRate)
			...
			Item pet = null;
			if (Utils.random(phaseKiller.getPerks().isUnlocked(Perks.CHANTER.getId()) ? 375 : 500) == 1) {
				pet = new Item(28630);
			} else if (phaseKiller.hasBombiChance() && Utils.random(phaseKiller.getPerks().isUnlocked(Perks.CHANTER.getId()) ? 185 : 250) == 1) {
				pet = new Item(33717);
			}
			if (pet != null && !phaseKiller.hasItem(pet)) {
				World.sendWorldMessage(Colors.orange + "<shad=000000><img=6>News: " + phaseKiller.getDisplayName() + " received a " + pet.getName() + " pet drop.", false);
				phaseKiller.addItem(pet);
				//new Thread(new NewsManager(phaseKiller, "<b><img src=\"../bin/images/news/" + image + "\" width=17> " + phaseKiller.getDisplayName() + " received a " + pet.getName() + " pet drop.")).start();
			}
		}
	...
	private void sendDrops(Player player, List<Drop> dropL, WorldTile location) {
		for (Drop drop : dropL) {
			...
			if (name.contains("seismic") || name.contains("stone")) {
				World.sendWorldMessage("<col=ff8c38><img=7>News: " + player.getDisplayName() + "" + (player.isIronMan() ? " (Iron Man) " : " ") + "has just received a " + name + " drop!" + "</col> ", false);
			}
			boolean stackable = ItemDefinitions.getItemDefinitions(drop.getItemId()).isStackable();
			Item item = new Item(drop.getItemId(), ((drop.getMinAmount()) + Utils.random(drop.getExtraAmount())) * bonus);
	...
	private void startNextPhase() {
		...
		if (hardMode && phase == 11) {
			...
			instance.getPlayersOnBattle().stream().filter(Objects::nonNull).forEachOrdered(player -> player.setNextAnimation(new Animation(20402)));
			WorldTasksManager.schedule(new WorldTask() {
				int count = 0;
	...
	private void sendCrashDownAttack(WorldTile crashDownTile) {
		...
		for (Player player : instance.getPlayersOnBattle()) {
			...
			}, (Objects.requireNonNull(AnimationDefinitions.getAnimationDefinitions(20338)).getEmoteClientCycles() / 30));
		}
	}
	...
	public void sendWaterFallAttack() {
		...
		WorldTile waterFallLocation = new WorldTile((Utils.random(2) == 0 ? (center.getX() - 10) : (center.getX() + 10)), (Utils.random(2) == 0 ? (center.getY() + 10) : (center.getY() - 10)), center.getPlane());
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < (4 - i); j++) {
				WorldTile tile = new WorldTile((rotation == 2 || rotation == 1) ? (xstart + j) : (xstart - j), (rotation == 0 || rotation == 1) ? (ystart - j) : (ystart + j), 1);
	...
	public void spawnStoneClones() {
		...
		for (int i = 0; i < maxStoneClones; i++) {
			...
			int id = combatType == CombatDefinitions.MAGIC_TYPE ? 17160 : combatType == CombatDefinitions.RANGE_TYPE ? 17159 : 17158;
			WorldTile spawnTile = instance.randomSpawnTile(null, new WorldTile(target.getX(), target.getY(), target.getPlane()), 4);
	...
	private void processBringHimDown() {
		if (getBringHimPoints() >= 200 && getTemporaryAttributtes().get("BringHimDownClick") == Boolean.TRUE) {
			...
			}, (Objects.requireNonNull(AnimationDefinitions.getAnimationDefinitions(20383)).getEmoteClientCycles() / 30));
			for (Player player : getVoragoInstance().getPlayersOnBattle()) {
				...
				}, (Objects.requireNonNull(AnimationDefinitions.getAnimationDefinitions(20400)).getEmoteClientCycles() / 30) + 1);
			}
		}
	}
	...
	private void finishBringHimDown() {
		for (Player player : instance.getPlayersOnBattle()) {
			if (player == null || player.isDead())
				continue;
			player.getPackets().sendPlayerMessage(1, 15263739, "<col=00ff00>Vorago stumbles! Deal as much damage as possible to loosen the next weapon piece!", true);
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				}, (Objects.requireNonNull(AnimationDefinitions.getAnimationDefinitions(20362)).getEmoteClientCycles() / 30));
				int finishType = (losenWeaponPieceStage >= 2 || damageWhileDown >= damageRequired) ? 2 : (damageWhileDown < (damageRequired / 2)) ? 1 : 0;
				for (Player player : instance.getPlayersOnBattle()) {
					if (player == null || player.isDead())
						continue;
					player.getPackets().sendGlobalConfig(1233, getVoragoInstance().getVorago().getBringHimPoints());
					player.getPackets().sendPlayerMessage(1, 15263739, (finishType == 2 ? "<col=00ff00>The weapon piece falls from Vorago's body!" : finishType == 1 ? "<col=ff0000>You didn't do enough damage in time. The weapon piece still looks secure." : "<col=FF6600>Your strong attacks helps loosen the weapon piece a little more."), true);
				}
				...
				public void run() {
					...
					case 4:
						...
						for (Player player : instance.getPlayersOnBattle()) {
							...
							int damage = underRago ? (distance == 0 ? 550 : (550 - (distance * 51))) : (distance > 5 ? 0 : (550 / distance == 0 ? 1 : distance));
							if (damage != 0) {
								if (damage < 150)
	...
	public void startTheEnd() {
		...
		WorldTile[] locations = { new WorldTile(voragoLocation.getX() + 7, voragoLocation.getY() + 7, voragoLocation.getPlane()), // north
																																	// east
		...
		int blueBombsAmount = (int) (hardMode ? (Math.max((getPlayerOnBattleCount() + 1), 7)) : ((getPlayerOnBattleCount() * 0.7) < 5 ? 5 : (getPlayerOnBattleCount() * 0.7)));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 10) {
					...
					for (int i = 0; i < 2; i++) {
						for (int j = 0; j < 20; j++) {
							int x = i == 0 ? (theEndStartIndex == 0 ? (center.getX() + j) : theEndStartIndex == 1 ? (center.getX()) : theEndStartIndex == 2 ? (center.getX() - j) : center.getX()) : (theEndStartIndex == 0 ? center.getX() : theEndStartIndex == 1 ? (center.getX() - j) : theEndStartIndex == 2 ? center.getX() : (center.getX() + j));
							int y = i == 0 ? (theEndStartIndex == 0 ? center.getY() : theEndStartIndex == 1 ? (center.getY() + j) : theEndStartIndex == 2 ? center.getY() : (center.getY() - j)) : (theEndStartIndex == 0 ? (center.getY() + j) : theEndStartIndex == 1 ? center.getY() : theEndStartIndex == 2 ? (center.getY() - j) : center.getY());
							int plane = getPlane();
					...
					for (int i = 0; i < 2; i++) {
						for (int j = 0; j < 20; j++) {
							int x = i == 0 ? (theEndStartIndex == 0 ? (center.getX() + j) : theEndStartIndex == 1 ? (center.getX()) : theEndStartIndex == 2 ? (center.getX() - j) : center.getX()) : (theEndStartIndex == 0 ? (center.getX() - j) : theEndStartIndex == 1 ? (center.getX()) : theEndStartIndex == 2 ? (center.getX() + j) : center.getX());
							int y = i == 0 ? (theEndStartIndex == 0 ? center.getY() : theEndStartIndex == 1 ? (center.getY() + j) : theEndStartIndex == 2 ? center.getY() : (center.getY() - j)) : (theEndStartIndex == 0 ? center.getY() : theEndStartIndex == 1 ? (center.getY() - j) : theEndStartIndex == 2 ? center.getY() : (center.getY() + j));
							int plane = getPlane();
							unWalkAbleSpots.add(new WorldTile(x, y, plane));
						}
					}
					...
					for (int i = 0; i < 2; i++) {
						for (int j = 0; j < 20; j++) {
							int x = i == 0 ? (nextFaceIndex == 0 ? (center.getX() + j) : nextFaceIndex == 1 ? (center.getX()) : nextFaceIndex == 2 ? (center.getX() - j) : center.getX()) : (nextFaceIndex == 0 ? center.getX() : nextFaceIndex == 1 ? (center.getX() - j) : nextFaceIndex == 2 ? center.getX() : (center.getX() + j));
							int y = i == 0 ? (nextFaceIndex == 0 ? center.getY() : nextFaceIndex == 1 ? (center.getY() + j) : nextFaceIndex == 2 ? center.getY() : (center.getY() - j)) : (nextFaceIndex == 0 ? (center.getY() + j) : nextFaceIndex == 1 ? center.getY() : nextFaceIndex == 2 ? (center.getY() - j) : center.getY());
							int plane = getPlane();
							unWalkAbleSpots.add(new WorldTile(x, y, plane));
						}
					}
	...
	public void sendPurpleBombAttack() {
		...
		int purpleBombCounter = 0;
		int playersCount = getPlayerOnBattleCount();
		int amountToLaunch = hardMode ? (phase >= 9 ? (Math.min(playersCount, 10)) : playersCount) : (phase == 5 ? (Math.min(playersCount, 5)) : playersCount);
		Vorago thisNPC = this;
		Collections.shuffle(instance.getPlayersOnBattle());
		for (int i = 0; i < amountToLaunch; i++) {
			...
			long arriveTime = startTime + (55000 + (25000L * purpleBombCounter));
			WorldTile fromLocation = purpleBomb != null ? purpleBomb : new WorldTile(player.getX() - 4, player.getY(), player.getPlane());
			double speed = Utils.getProjectileSpeed(fromLocation, player, purpleBomb != null ? 5 : 250, 20, startTime, arriveTime);
			Projectile projectile = World.sendProjectileNew(fromLocation, player, 5326, purpleBomb != null ? 5 : 250, 20, 10, speed, 0, 0);
			...
			CoresManager.fastExecutor.scheduleAtFixedRate(new TimerTask() {
				...
				public void run() {
					...
					if (count == 0) {
						...
						CombatScript.delayHit(thisNPC, 0, player, new Hit(thisNPC, damage, HitLook.VORAGO_SPECIAL_DAMAGE));
	...
	private boolean isBetweenVoragoHands(WorldTile tile) {
		...
		int smallestX = Math.min(center.getX(), cornerLocation.getX());
		int largestX = Math.max(center.getX(), cornerLocation.getX());
		int smallestY = Math.min(center.getY(), cornerLocation.getY());
		int largestY = Math.max(center.getY(), cornerLocation.getY());
		...
		for (int bombObjectId : bombObjectIds) {
			for (int x = smallestX; x < largestX; x++) {
				for (int y = smallestY; y < largestY; y++) {
					TheEndBomb bomb = (TheEndBomb) World.getObjectWithId(new WorldTile(x, y, cornerLocation.getPlane()), bombObjectId);
					if (bomb == null)
						continue;
					return bomb;
				}
	...
	public void sendTeamSplit() {
		...
		for (int i = 0; i < playersCount; i += 2) {
			...
			player.getPackets().sendGameMessage("You are in team " + (redTeam ? "<col=ff0000>RED" : "<col=00ff00>GREEN") + "</col> , hurry and get in square.");
			playersInTeam++;
		}
		...
		for (int i = playersLeft; i > 0; i--) {
			for (Player player : players) {
				if (player == null || player.isDead() || player.getTemporaryAttributtes().get("TeamSplitRed") != null)
					continue;
				player.getTemporaryAttributtes().put("TeamSplitRed", !redTeam);
				player.getPackets().sendGameMessage("You are in team " + (redTeam ? "<col=00ff00>GREEN" : "<col=ff0000>RED") + "</col> , hurry and get in square.");
				break;
			}
		}
		for (Player player : instance.getPlayersOnBattle()) {
			...
			public void run() {
				if (getHitpoints() == 0) {
					for (int i = 0; i < teamSplitSquares.length; i++) {
						...
						if (square == null)
							continue;
						if (World.containsObjectWithId(new WorldTile(square), 88882) || World.containsObjectWithId(new WorldTile(square), 88881))
							World.removeObject(square);
						teamSplitSquares[i] = null;
					}
	...
	public void sendGreenBomb() {
		...
		WorldTile voragoSquare = ((!hardMode && phase == 5) || (hardMode && phase >= 10)) ? new WorldTile(this) : getVoragoSquare();
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if ((!hardMode && phase != 5) || (hardMode && phase < 10)) {
					...
					if (index != -1) {
						mists[index] = new WorldObject(87322, 10, 0, new WorldTile(mistLocation.getX() - 5, mistLocation.getY() - 3, mistLocation.getPlane()));
				...
				int angle = (int) Math.round(Math.toDegrees(Math.atan2((mistLocation.getX() * 2 + 1) - (greenBombP.getX() * 2 + greenBombP.getSize()), (mistLocation.getY() * 2 + 1) - (greenBombP.getY() * 2 + greenBombP.getSize()))) / 45d) & 0x7;
				Projectile projectile = World.sendProjectileNew(mistLocation, new WorldTile(greenBombP), 4269, 0, 10, 10, speed, angle, 0);
				int cycleTime = Utils.projectileTimeToCycles(projectile.getEndTime()) - 1;
				WorldTasksManager.schedule(new WorldTask() {
					int count = 0;
					...
					public void run() {
						...
						if (count == startTime) {
							if (getPlayersNearby(greenBombP, 2) == 0 || (greenBombP.getTemporaryAttributtes().get("GreenBombed") != null && greenBombBounces != 1)) {
							...
							double speed = 0.02 + (distance / 24.00);
							int angle = (int) Math.round(Math.toDegrees(Math.atan2((greenBombP.getX() * 2 + greenBombP.getSize()) - (p2.getX() * 2 + p2.getSize()), (greenBombP.getY() * 2 + greenBombP.getSize()) - (p2.getY() * 2 + p2.getSize()))) / 45d) & 0x7;
							Projectile projectile = World.sendProjectileNew(new WorldTile(greenBombP), new WorldTile(p2), 4269, 10, 30, 10, speed, angle, 0);
							int cycleTime = Utils.projectileTimeToCycles(projectile.getEndTime()) - 1;
							count = 0;
							startTime = cycleTime;
	...
	private Player getRandomTarget(WorldTile tile, Player exception, int maxDistance) {
		...
		for (int i = 0; i < instance.getPlayersOnBattle().size(); i++) {
			Player player = instance.getPlayersOnBattle().get(i);
			if (player == null || player.isDead() || (exception != null && player == exception) || Utils.getDistance(tile, player) > maxDistance)
				continue;
	...
	public boolean sendVitalisOrb() {
		boolean hardMode = instance.getSettings().isHardMode();
		if ((((!hardMode && phase == 5) || (hardMode && phase == 10)) && !World.checkWalkStep(getPlane(), getX() - 5, getY() + 2, 1, 1)) || (hardMode && phase == 11 && !World.checkWalkStep(getPlane(), getX() + 5, getY() + 2, 1, 1)))
			return false;
		setNextAnimation(new Animation(20328));
		setNextGraphics(new Graphics(4026));
		WorldTile closestVitalisSpawn = (((!hardMode && phase == 5) || (hardMode && phase == 10)) ? new WorldTile(getX() - 5, getY() + 2, getPlane()) : (hardMode && phase == 11) ? new WorldTile(getX() + 5, getY() + 2, getPlane()) : getVitalisSpawnLocation());
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (Player player : instance.getPlayersOnBattle()) {
					if (player == null || player.isDead() || !isInSquare(player, closestVitalisSpawn.getX() - 2, closestVitalisSpawn.getX() + 2, closestVitalisSpawn.getY() - 2, closestVitalisSpawn.getY() + 2))
						continue;
					vitalisCount--;
	...
	public void spawnScopuli() {
		...
		WorldTile[] scopuliLocations = { new WorldTile(center.getX() - 2, center.getY() - 4, center.getPlane()), new WorldTile(center.getX() + 8, center.getY() - 4, center.getPlane()), new WorldTile(center.getX() + 8, center.getY() + 4, center.getPlane()) };
	...
	public void fireGravityField(Player player) {
		...
		if (gravityField == null)
			return;
		if (!Utils.isOnRange(new WorldTile(gravityField.getX(), gravityField.getY(), gravityField.getPlane()), this, 7, 1, getSize())) {
		...
		player.lock(3);
		Projectile projectile = World.sendProjectileNew(new WorldTile(gravityField.getX(), gravityField.getY(), 0), this, 4029, 0, 30, 10, 0.5, 30, 1);
		int cycleTime = Utils.projectileTimeToCycles(projectile.getEndTime()) - 1;
		World.removeObject(gravityField);
		phaseProgress++;
		gravityField = null;
