...
public class DungManager implements Serializable {
	...
	private static enum ResourceDungeon {
		EDGEVILLE_DUNGEON(10, 1100, 52849, new WorldTile(3132, 9933, 0), 52867,
				new WorldTile(991, 4585, 0)), DWARVEN_MINE(15, 1500, 52855, new WorldTile(3034, 9772, 0), 52864,
						new WorldTile(1041, 4575, 0)), EDGEVILLE_DUNGEON_2(20, 1600, 52853,
								new WorldTile(3104, 9826, 0), 52868, new WorldTile(1135, 4589, 0)), KARANJA_VOLCANO(25,
										2100, 52850, new WorldTile(2845, 9557, 0), 52869,
										new WorldTile(1186, 4598, 0)), DAEMONHEIM_PENINSULA(30, 2400, 52861,
												new WorldTile(3513, 3666, 0), 52862,
												new WorldTile(3498, 3633, 0)), BAXTORIAN_FALLS(35, 3000, 52857,
														new WorldTile(2578, 9898, 0), 52873,
														new WorldTile(1256, 4592, 0)), MINING_GUILD(45, 4400, 52856,
																new WorldTile(3022, 9741, 0), 52866,
																new WorldTile(1052, 4521, 0)), TAVERLY_DUNGEON_1(55,
																		6200, 52851, new WorldTile(2854, 9841, 0),
																		52870,
																		new WorldTile(1394, 4588,
																				0)), TAVERLY_DUNGEON_2(60, 7000, 52852,
																						new WorldTile(2912, 9810, 0),
																						52865,
																						new WorldTile(1000, 4522,
																								0)), VARRICK_SEWERS(65,
																										8500, 52863,
																										new WorldTile(
																												3164,
	...
	public int getTokens() {
		return player.getDungeoneeringTokens();
	}
	...
	public void openPartyInterface() {
		player.getTemporaryAttributtes().remove("DUNG_INSPECTED_PLAYER");
		player.getTemporaryAttributtes().remove("DUNG_INSPECTED_TAB");
	...
	// 936 Skills interface, 946 Familiar Bob Interface, 954 Gear interface,
	// 953 inventory

	public void openInspectInterface(int tab) {
		player.getInterfaceManager().sendDungPartyTab(tab == 0 ? 953 : tab == 1 ? 954 : tab == 2 ? 946 : 936);
		player.getInterfaceManager().openDungTab();
		player.getTemporaryAttributtes().put("DUNG_INSPECTED_TAB", tab);
		refreshInspectDetails();
	}

	public void closeInspectInterface() {
		player.getTemporaryAttributtes().remove("DUNG_INSPECTED_PLAYER");
		player.getTemporaryAttributtes().remove("DUNG_INSPECTED_TAB");
		openPartyInterface();
	}

	private void refreshInspectDetails() {
		Player inspected = (Player) player.getTemporaryAttributtes().get("DUNG_INSPECTED_PLAYER");
		if (inspected == null)
			return;
		int tab = (int) player.getTemporaryAttributtes().get("DUNG_INSPECTED_TAB");
		player.getPackets().sendIComponentText(tab == 0 ? 953 : tab == 1 ? 954 : tab == 2 ? 946 : 936,
				tab == 0 ? 6 : tab == 1 ? 5 : tab == 2 ? 8 : 132, inspected.getDisplayName() + "'s "
						+ (tab == 0 ? "Inventory" : tab == 1 ? "Worn" : tab == 2 ? "Bob" : "Stats"));
		player.getPackets().sendUnlockIComponentOptionSlots(953, 0, 0, 27, 0, 1, 2);
		player.getPackets().sendInterSetItemsOptionsScript(953, 0, 93, true, 4, 7, "Examine");
		// Inventory
		player.getPackets().sendItems(93, true, inspected.getInventory().getItems());

		// Equipment
		for (int i = Equipment.SLOT_HAT; i <= Equipment.SLOT_SHIELD; i++) {
			Item item = inspected.getEquipment().getItem(i);
			if (item != null)
				player.getPackets().sendItemOnIComponent(954, (18 + (i * 3)), item.getId(), item.getAmount());
		}
		Item item = inspected.getEquipment().getItem(Equipment.SLOT_LEGS);
		if (item != null)
			player.getPackets().sendItemOnIComponent(954, 36, item.getId(), item.getAmount());
		item = inspected.getEquipment().getItem(Equipment.SLOT_HANDS);
		if (item != null)
			player.getPackets().sendItemOnIComponent(954, 39, item.getId(), item.getAmount());
		item = inspected.getEquipment().getItem(Equipment.SLOT_FEET);
		if (item != null)
			player.getPackets().sendItemOnIComponent(954, 42, item.getId(), item.getAmount());
		item = inspected.getEquipment().getItem(Equipment.SLOT_RING);
		if (item != null)
			player.getPackets().sendItemOnIComponent(954, 45, item.getId(), item.getAmount());
		item = inspected.getEquipment().getItem(Equipment.SLOT_ARROWS);
		if (item != null)
			player.getPackets().sendItemOnIComponent(954, 49, item.getId(), item.getAmount());

		// Familiar
		if (inspected.getFamiliar() != null && inspected.getFamiliar().getBob() != null)
			player.getPackets().sendItems(530, true, inspected.getFamiliar().getBob().getBeastItems());

		// Stats
		for (int i = 7; i <= 127; i += 5) {
			int skill = getSkillForComponentId(i);
			player.getPackets().sendIComponentText(936, i, "" + inspected.getSkills().getLevel(skill));
			player.getPackets().sendIComponentText(936, i + 1, "" + inspected.getSkills().getLevelForXp(skill));
		}
	}

	public int getSkillForComponentId(int componentId) {
		switch (componentId) {
		case 7:
	...
	public void invite(String name) {
		...
		if (party == null) {
			...
			if (p2.getDungManager().invitingPlayer != player || player.getPlane() != 0 || party == null
					|| !party.isLeader(p2)) {
				player.getPackets()
						.sendGameMessage((p2.getDungManager().invitingPlayer != player || player.getPlane() != 0)
								? "You can't do that right now, Make sure your in a party and your the leader."
								: party == null ? "You need to be in a party to do that."
										: "Only the leader can invite players.");
				return;
			}
