...
public class PlayerOwnedPort implements Serializable {
	...
	public boolean handleFirstShipReward() {
		...
		if (random < (player.getPerks().isUnlocked(Perks.PORTS.getId())? 8 : 10)) { //10% chance of returning unsuccessfully
			chime += resources / (player.getPerks().isUnlocked(Perks.PORTS.getId()) ? 2 : 3);
			return false;
		}
		if (player.getPerks().isUnlocked(Perks.PORTS.getId()))
			resources *= 1.15;
