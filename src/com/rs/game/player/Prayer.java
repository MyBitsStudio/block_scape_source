...
public class Prayer implements Serializable {
	...
	public Prayer() {
		quickPrayers = new boolean[][] { new boolean[30], new boolean[25] };
		prayerpoints = 10;
	}
	...
	public void closeAllPrayers() {
		onPrayers = new boolean[][] { new boolean[30], new boolean[25] };
	...
	public void drainPrayer(int amount) {
		if (player.getPerks().isUnlocked(Perks.BETRAYER.getId())) {
			if (Utils.random(6) < 3)
				return;
			if (amount > 2)
				amount = amount / 2;
		}
		if (player.getPetManager().isPrayerPet()) {
			if (Utils.random(6) < 3)
				return;
			if (amount > 2)
				amount = amount / 2;
	...
	public void drainPrayerOnHalf() {
		...
		if (prayerpoints > 0) {
			prayerpoints = prayerpoints / (player.getPerks().isUnlocked(Perks.BETRAYER.getId()) ? 4 : 2);
			refreshPrayerPoints();
		}
	}
	...
	public double getAttackMultiplier() {
		...
		if (usingPrayer(0, 2))
			...
			double d = (17 + leechBonuses[8]);
			value += d / 100;
		}
		if (player.getPetManager().isPrayerPet() && player.isAtWild() && !player.isCanPvp()) {
			for (int index = 0; index < player.getPet().getDetails().getLevel(); index++)
				value += 0.02;
		}
		return value;
	}
	...
	public double getDefenceMultiplier() {
		...
		if (usingPrayer(0, 0))
			...
			double d = (15 + leechBonuses[9]);
			value += d / 100;
		} else if (usingPrayer(1, 21)) {
			double d = (15 + leechBonuses[9]);
			value += d / 100;
		} else if (usingPrayer(1, 22)) {
			double d = (17 + leechBonuses[9]);
			value += d / 100;
	...
	public double getRangeMultiplier() {
		...
		if (usingPrayer(0, 3))
			...
			double d = (15 + leechBonuses[4]);
			value += d / 100;
		} else if (usingPrayer(1, 23)) {
			double d = (17 + leechBonuses[4]);
			value += d / 100;
		}
		return value;
	}
	...
	public void increaseTurmoilBonus(Entity e, int type) {
		if (e instanceof Player) {
			Player p2 = (Player) e;
			if (type == 0 || type == 3) {
				leechBonuses[8] = (int) ((100
						* Math.floor((type == 3 ? 0.17 : 0.15) * p2.getSkills().getLevelForXp(Skills.ATTACK)))
						/ p2.getSkills().getLevelForXp(Skills.ATTACK));
				leechBonuses[9] = (int) ((100
						* Math.floor((type == 3 ? 0.17 : 0.15) * p2.getSkills().getLevelForXp(Skills.DEFENCE)))
						/ p2.getSkills().getLevelForXp(Skills.DEFENCE));
				leechBonuses[10] = (int) ((100 
						* Math.floor(0.1 * p2.getSkills().getLevelForXp(Skills.STRENGTH)))
			...
			if (type == 1 || type == 4) {
				leechBonuses[4] = (int) ((100
						* Math.floor((type == 4 ? 0.17 : 0.15) * p2.getSkills().getLevelForXp(Skills.RANGE)))
						/ p2.getSkills().getLevelForXp(Skills.RANGE));
				leechBonuses[9] = (int) ((100
						* Math.floor((type == 4 ? 0.17 : 0.15) * p2.getSkills().getLevelForXp(Skills.DEFENCE)))
						/ p2.getSkills().getLevelForXp(Skills.DEFENCE));
				adjustStat(2, leechBonuses[9]);
				adjustStat(3, leechBonuses[4]);
			}
			if (type == 2 || type == 5) {
				leechBonuses[5] = (int) ((100
						* Math.floor((type == 5 ? 0.17 : 0.15) * p2.getSkills().getLevelForXp(Skills.MAGIC)))
						/ p2.getSkills().getLevelForXp(Skills.MAGIC));
				leechBonuses[9] = (int) ((100
						* Math.floor((type == 5 ? 0.17 : 0.15) * p2.getSkills().getLevelForXp(Skills.DEFENCE)))
						/ p2.getSkills().getLevelForXp(Skills.DEFENCE));
				adjustStat(2, leechBonuses[9]);
			...
			NPC n = (NPC) e;
			if (type == 0 || type == 3) {
				int atkLevel = n.getBonuses() == null ? 80 : n.getBonuses()[0];
				int strLevel = n.getBonuses() == null ? 80 : n.getBonuses()[2];
				int defLevel = n.getBonuses() == null ? 80 : n.getBonuses()[5];
				leechBonuses[8] = (int) ((100 * Math.floor((type == 3 ? 0.17 : 0.15) * atkLevel)) / atkLevel);
				leechBonuses[9] = (int) ((100 * Math.floor((type == 3 ? 0.17 : 0.15) * defLevel)) / defLevel);				
				//leechBonuses[10] = (int) ((100 * Math.floor(0.1 * strLevel)) / strLevel);
				leechBonuses[10] = (int) ((100 * Math.floor((type == 3 ? 0.125 : 0.1)  * strLevel)) / strLevel);
				adjustStat(0, leechBonuses[8]);
				adjustStat(1, leechBonuses[10]);
				adjustStat(2, leechBonuses[9]);
			}
			if (type == 1 || type == 4) {
				int rangedLevel = n.getBonuses() == null ? 80 : n.getBonuses()[4];
				int defLevel = n.getBonuses() == null ? 80 : n.getBonuses()[5];
				leechBonuses[4] = (int) ((100 * Math.floor((type == 4 ? 0.17 : 0.15) * rangedLevel)) / rangedLevel);
				leechBonuses[9] = (int) ((100 * Math.floor((type == 4 ? 0.17 : 0.15) * defLevel)) / defLevel);
				adjustStat(2, leechBonuses[9]);
				adjustStat(3, leechBonuses[4]);
			}
			if (type == 2 || type == 5) {
				int mageLevel = n.getBonuses() == null ? 80 : n.getBonuses()[3];
				int defLevel = n.getBonuses() == null ? 80 : n.getBonuses()[5];
				leechBonuses[5] = (int) ((100 * Math.floor((type == 5 ? 0.17 : 0.15) * mageLevel)) / mageLevel);
				leechBonuses[9] = (int) ((100 * Math.floor((type == 5 ? 0.17 : 0.15) * defLevel)) / defLevel);
				adjustStat(2, leechBonuses[9]);
				adjustStat(4, leechBonuses[5]);
			}
		}
	...
	private void recalculatePrayer() {
		...
		player.getVarsManager()
				.sendVar(ancientcurses ? (usingQuickPrayer ? 1587 : 1582) : (usingQuickPrayer ? 1397 : 1395), value);
	}
	...
	public void resetDrainPrayer(int index) {
		nextDrain[index] = (long) (Utils.currentTimeMillis() + (prayerDrainRate[getPrayerBook()][index] * 1000)
				+ (player.getCombatDefinitions().getBonuses()[CombatDefinitions.PRAYER_BONUS] * 50L));
	}
