...
public final class Skills implements Serializable {
	...
	private HashMap<String, Boolean> spinMap;
	...
	public double getModifiedXP(int skill, double exp) {
		...
		if (player.getPlayerVars().hasBonusEXP()) {
			try {
				exp *= (Double.parseDouble(getPercentage()) / 100) + 1;
				if (Settings.SUPERLOG)
					Logger.log("[VOTE BOOK XP]: " + player.getDisplayName() + "'s modifier: "
							+ Double.parseDouble(getPercentage()) / 100);
			} catch (Exception e) {
				System.err.println("Wrongful var parsed as byte.");
			}
		}
		...
		if (skill == Skills.DIVINATION) {
			if (player.getPerks().isUnlocked(Perks.DIVINER.getId()))
				exp *= 1.25;
		}
		if (skill == Skills.HUNTER) {
			if (player.getPerks().isUnlocked(Perks.HUNTSMAN.getId()))
				exp *= 1.25;
		}
	...
	public void addXp(int skill, double exp, boolean forceRSXP, boolean xpShare) {
		...
		if (!(manager == null) && !(isCombatSkill(skill))) {
			if (player.isEasy())
				player.getClanManager().getClan().getExperienceType().addDefaultLevelingExp(player, (int) exp / 30);
			if (player.isIntermediate())
				player.getClanManager().getClan().getExperienceType().addDefaultLevelingExp(player, (int) exp / 30);
			if (player.isVeteran())
				player.getClanManager().getClan().getExperienceType().addDefaultLevelingExp(player, (int) exp / 20);
			if (player.isExpert())
				player.getClanManager().getClan().getExperienceType().addDefaultLevelingExp(player, (int) exp / 20);
		...
		if (player.getControlerManager().getControler() instanceof CitadelControler) {
			if (!(manager == null) && !(isCombatSkill(skill)))
				player.getClanManager().getClan().getExperienceType().addDefaultLevelingExp(player, (int) exp / 10);
		}
		if (player.getPet() != null)
			player.getPet().addExperience(exp / 6);
		...
		if (oldXP < 1000000000 && xp[skill] == 1000000000)
			LevelUp.send1000m(player, skill);
		if (oldXP < 2000000000 && xp[skill] == MAXIMUM_EXP)
			LevelUp.send2000m(player, skill);
	...
	public int getLevelForXp(int skill) {
		...
		return skill == DUNGEONEERING || skill == SLAYER ? 120 : 99;
	...
	public HashMap<String, Boolean> getSpinMap() {
		return spinMap;
	}

	public void setSpinMap(HashMap<String, Boolean> spinMap) {
		this.spinMap = spinMap;
	}
