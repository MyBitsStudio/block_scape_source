...
public class SlayerManager implements Serializable {
	...
	private final SlayerTask[] canceledTasks;
	private SlayerTask currentTask;
	private SlayerMaster currentMaster;
	protected int completedTasks, slayerPoints, maximumTaskCount, currentTaskCount, slayerPoints2;
	private final boolean[] learnedAbilities;
	...
	public void setPlayer(Player player) {
		...
		if (player.getSlayerPoints() > 0) {
			this.slayerPoints = player.getSlayerPoints();
			player.setSlayerPoints(0);
		}
		if (player.getSlayerPoints2() > 0) {
			this.slayerPoints2 = player.getSlayerPoints2();
			player.setSlayerPoints2(0);
		}
	}
	...
	private void cancleCurrentTask() {
		if (currentTask == null) {
			...
			if (player.getPerks().isUnlocked(Perks.PERSLAY.getId())|| removePoints(30)) {
				skipCurrentTask();
				setCurrentTask(true);
	...
	public void checkCompletedTask(int damageAdmitted, int otherDamageAdmitted) {
		...
		Pets.checkSkillingPet(player, 38105);
	...
	public void invitePlayer(Player otherPlayer) {
		...
		otherPlayer.getPackets().sendMessage(117, "You have received an invitation from the player "
				+ player.getDisplayName() + " for either social slayer or clan recruitement.", player);
	}
	...
	public void setCurrentTask(boolean initial) {
		...
		double nerf = 0.60; // 40% less task
		int unnerfed = (int) futureTask[1];
		int total = (int) (unnerfed * nerf);
		setCurrentTask((SlayerTask) futureTask[0], total);
		checkKillsLeft();
		if (initial) {
			if (socialPlayer != null)
				socialPlayer.getSlayerManager().setCurrentTask((SlayerTask) futureTask[0], total);
		}
	}
	...
	public void setPoints2(int i) {// admin only
		this.slayerPoints2 = i;
	}
