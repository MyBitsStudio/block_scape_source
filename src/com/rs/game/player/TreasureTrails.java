...
public class TreasureTrails implements Serializable {
	...
	public void setNextClue(int source) {
		int lastPhase = (currentClue.details.type == COORDINATE && currentClue.dificulty >= HARD) ? 2 : (currentClue.details.type == EMOTE ? 5 : 0); // last phase(5) get's set by the dialog
		...
		if (cluePhase == lastPhase) {
			...
			if (source == SOURCE_DIG) {
				...
				if (currentClue.isLast())
					currentClue.count--; // so if it's this case then we will extend your trail ahahaha kleared kkk9k9k9k999kggg
				...
				if (currentClue.isLast())
					currentClue.count--; // so if it's this case then we will extend your trail ahahaha kleared kkk9k9k9k999kggg
				currentClue.details = generateClueDetails(currentClue.dificulty);// don't discount, just regen
			...
			if (cluePhase == 0 && currentClue.dificulty >= HARD) {
				// spawn combat npc
				...
				cluePhase = 1;
			} else if ((cluePhase == 0 || cluePhase == 2 && currentClue.dificulty >= HARD) && currentClue.details.type == EMOTE) {
				// spawn ugi
	...
	public boolean useItem(Item item) {
		...
		if (level != -1) {
			if(player.getPerks().isUnlocked(Perks.INVESTIGATOR.getId())) {
	...
	public boolean useDig() {
		...
		if ((currentClue.details.type == SIMPLE || currentClue.details.type == MAP) && currentClue.details.idType == TILE) {
			...
			if (!player.withinDistance(t, 6)) //setted distance cuz the getTile method may miss 3-5 tiles on rs
				return false;
			setNextClue(SOURCE_DIG);
			return true;
		}
	...
	public void openReward(int level) {
		...
		player.setCloseInterfacesEvent(() -> {
			for (Item item : rewards) {
				...
				if (name.contains("dye") || name.contains("third-age") || name.contains("robin hood")
										 || name.contains("second-age")) {
					World.sendWorldMessage(Colors.orange + "<shad=000000><img=6>News: "
							+ player.getDisplayName() + " received "+Utils.getAorAn(name) + " "+ name + " from a clue scroll!", false);

					/*new Thread(new NewsManager(player, "<b><img src=\"./bin/images/news/clue_scroll.png\" height=14> "
						+ player.getDisplayName()+" received "+Utils.getAorAn(name) + " "+ name + " from a clue scroll.")).start();*/
				}
			}
		});
		player.incrementCompletedClues();
		player.sendMessage("Well done! You've completed the Treasure Trail; "
				+ "trails done: "+Colors.red+Utils.getFormattedNumber(player.getCompletedClues())+"</col>.");
	}
	...
	private int[] getCoordinates(WorldTile tile) {
		...
		minY -= degreeY * 60;
		return new int[]
		{ degreeY, minY, dirY, degreeX, minX, dirX };
	}
	...
	private static int[] createMix() {
		...
		if ((inversions & 1) != 0) {
			return createMix(); //could just simply swap last 2 pieces but need to make sure they arn't blank
		}
		return mix;
	}
	...
	private enum ClueDetails {
		...
		SIMPLE_S_11(EASY, SIMPLE, OBJECT, 12104, "Search the boxes in the house", "near the South entrance of", "Varrock."),
		...
		//SIMPLE_S_37(EASY, SIMPLE, OBJECT, 25035, "Search the drawers in Catherby's", "Archery Shop."),   //33931
		...
		ANAGRAM_G_1(MEDIUM, ANAGRAM, NPC, 469, "Goblin Kem"),
		...
		EMOTE_B_1(MEDIUM, EMOTE, TILE, 45829116, new int[]
		{ 10, 22 }, new Item[]
		//{ new Item(20922, 1), new Item(1099, 1), new Item(1143, 1) }, "Beckon in Tai Bwo Wannai.", "Clap before you talk to me.", " Equip green dragonhide chaps,", "a ring of duelling, and a mithril med helm."),
		//EMOTE_B_2(MEDIUM, EMOTE, TILE, 55184739, new int[]
		//{ 10, 4 }, new Item[]
		//{ new Item(3329, 1), new Item(6328, 1), new Item(6328, 1) }, "Beckon in the Digsite, near", "the eastern winch.", " Bow or curtsy before you talk to me.", "Equip a pointed red and black snelm (round),", "snakeskin boots, and an iron pickaxe."),
		//EMOTE_B_3(HARD, EMOTE, TILE, 46730121, new int[]
		//{ 19 }, new Item[]
		//{ new Item(3385, 1), new Item(7170, 1), new Item(1127, 1) }, "Blow a kiss between the tables", "in Shilo Village bank.", "Beware of double agents!", "Equip a splitbark helmet, mud pie", "and rune platebody."),
		//EMOTE_B_4(EASY, EMOTE, TILE, 42732751, new int[]
		//{ 21 }, new Item[]
		{ new Item(1133, 1), new Item(1075, 1), new Item(7170, 1) }, "Blow a raspberry at the monkey", "cage in Ardougne Zoo.", "Equip a studded body,", "bronze platelegs and a mud pie."),
		EMOTE_B_5(HARD, EMOTE, TILE, 42372446, new int[]
		{ 21 }, new Item[]
		{ new Item(2890, 1), new Item(2493, 1), new Item(1347, 1) }, "Blow a raspberry in", "the Fishing Guild bank.", "Beware of double agents!", "Equip an elemental shield, blue ", "dragonhide chaps, and a rune warhammer."),
		EMOTE_B_6(EASY, EMOTE, TILE, 45272394, new int[]
		{ 21 }, new Item[]
		{ new Item(1169, 1), new Item(1115, 1), new Item(1059, 1) }, "Blow raspberries outside", "the entrance to Keep Le Faye.", "Equip a coif, an iron platebody", "and leather gloves."),
		EMOTE_B_7(HARD, EMOTE, TILE, 577982012, new int[]
		{ 4 }, new Item[]
		{ new Item(2499, 1), new Item(2487, 1) }, "Bow or curtsy at the", "top of the lighthouse.", "Beware of double agents!", "Equip a blue dragonhide body,", "blue dragonhide vambraces", "and no jewellery."),
		EMOTE_B_8(EASY, EMOTE, TILE, 55200971, new int[]
		{ 4 }, new Item[]
		{ new Item(1101, 1), new Item(1095, 1), new Item(1169, 1) }, "Bow or curtsy in the lobby", "of the Duel Arena.", "Equip an iron chainbody,", "leather chaps and a coif."),
		EMOTE_B_9(EASY, EMOTE, TILE, 44715285, new int[]
		{ 4 }, new Item[]
		{ new Item(1067, 1), new Item(1696, 1), new Item(845, 1) }, "Bow or curtsy outside", "the entrance to the Legends' Guild.", "Equip iron platelegs,", "an emerald amulet and oak longbow."),
		EMOTE_C_1(EASY, EMOTE, TILE, 36164446, new int[]
		//{ 9 }, new Item[] {}, "Cheer at the Burthrope Games Room.", "Have nothing equipped at all when you do."),
		//EMOTE_C_2(EASY, EMOTE, TILE, 47648172, new int[]
		{ 9 }, new Item[]
		{ new Item(5527, 1), new Item(1307, 1), new Item(1692, 1) }, "Cheer at the druids circle,", "equip an Air tiara, a bronze 2h sword,", "and Gold amulet."),
		EMOTE_C_3(EASY, EMOTE, TILE, 49941668, new int[]
		{ 9 }, new Item[]
		{ new Item(1169, 1), new Item(1083, 1), new Item(1656, 1) }, "Cheer for the monks at Port Sarim.", "Equip a coif, steel plateskirt", "and a sapphire necklace."),
		EMOTE_C_4(MEDIUM, EMOTE, TILE, 41799140, new int[]
		{ 9, 17 }, new Item[]
		{ new Item(1119, 1), new Item(853, 1), new Item(4119, 1) }, "Cheer in the Barbarian Agility Arena.", "Headbang before you talk to me.", "Equip a steel platebody, maple, ", "shortbow and bronze boots."),
		EMOTE_C_5(MEDIUM, EMOTE, TILE, 41422128, new int[]
		{ 9, 5 }, new Item[]
		{ new Item(1135, 1), new Item(1099, 1), new Item(1177, 1) }, "Cheer in the Ogre pen", "in the Training Camp.", "Show you are angry before you talk", "to me. Equip a Green Dragonhide Body, ", "green dragonhide chaps", "and a steel sq shield."),
		EMOTE_C_6(EASY, EMOTE, TILE, 55086348, new int[]
		{ 22 }, new Item[]
		{ new Item(1698, 1), new Item(2464, 1), new Item(1059, 1) }, "Clap in the main exam room", "in the Exam Centre.", "Equip a ruby amulet, blue flowers", "and leather gloves."),
		EMOTE_C_7(EASY, EMOTE, TILE, 50826366, new int[]
		{ 22 }, new Item[]
		{ new Item(1137, 1), new Item(1639, 1), new Item(1059, 1) }, "Clap on the causeway to", "the Wizard's Tower.", " Equip an Iron helm, emerald ring", "and leather gloves."),
		EMOTE_C_8(EASY, EMOTE, TILE, 579980602, new int[]
		{ 22 }, new Item[]
		{ new Item(1639, 1), new Item(1119, 1), new Item(5525, 1) }, "Clap on the top level of the mill,", "north of east Ardougne.", "Equip an Emerald ring,", "Steel platebody and an uncharged tiara."),
		EMOTE_C_9(MEDIUM, EMOTE, TILE, 46255474, new int[]
		{ 18, 4 }, new Item[]
		//{ new Item(3329, 1), new Item(1131, 1), new Item(2961, 1) }, "Cry in Catherby archery shop.", "Bow or curtsy before you talk to me.", "Equip a round red and black snelm,", "a hardleather body and an unblessed", "silver sickle."),
		//EMOTE_C_10(MEDIUM, EMOTE, TILE, 577424732, new int[]
		//{ 18, 3 }, new Item[]
		//{ new Item(1099, 1), new Item(1193, 1), new Item(2568, 1) }, "Cry on the platform of", "the north-west tree of the", "Gnome Agility Arena. Indicate 'no'", "before talking to me,", "equip green d'hide chaps, a steel", "kiteshield, and a ring of forging."),
		//EMOTE_D_1(MEDIUM, EMOTE, TILE, 54152242, new int[]
		//{ 15, 4 }, new Item[]
		//{ new Item(3343, 1), new Item(1381, 1) }, "Dance a jig under Shantay's Awning.", "Bow or curtsy before you talk to me.", "Equip a pointed blue snail", "helmet and an air staff."),
		//EMOTE_D_2(HARD, EMOTE, TILE, 53971677, new int[]
		//{ 14, }, new Item[]
		//{ new Item(2570, 1), new Item(1704, 1), new Item(1317, 1) }, "Dance at the cat-doored", "pyramid in Sophanem.", "Beware of double agents!", "Equip a ring of life,", "an uncharged amulet of glory,", "and an adamant 2h sword."),
		//EMOTE_D_3(EASY, EMOTE, TILE, 50957536, new int[]
		//{ 14 }, new Item[]
		//{ new Item(1101, 1), new Item(1637, 1), new Item(839, 1) }, "Dance at the crossroads", "north of Draynor,", "equip an iron chainbody,", "a sapphire ring and a longbow. "),
		//EMOTE_D_4(MEDIUM, EMOTE, TILE, 57281956, new int[]
		//{ 14, 4 }, new Item[]
		//{ new Item(4551, 1), new Item(1071, 1), new Item(1309, 1) }, "Dance in the centre of Canifis.", "Bow or curtsy before you talk to me.", "Equip a spiny helm,", "mithril platelegs, and an", "iron two-handed sword."),
		//EMOTE_D_5(EASY, EMOTE, TILE, 49892657, new int[]
		//{ 14 }, new Item[]
		//{ new Item(1157, 1), new Item(1119, 1), new Item(1081, 1) }, "Dance in the Party Room.", "Equip a steel full helm,", "steel platebody and, ", "iron plateskirt."),
		//EMOTE_D_6(EASY, EMOTE, TILE, 52481120, new int[]
		//{ 14 }, new Item[]
		{ new Item(1205, 1), new Item(1153, 1), new Item(1635, 1) }, "Dance in the shack", "in Lumbridge Swamp.", "Equip a bronze dagger,", "iron full helm and", "a gold ring."),
		EMOTE_H_1(EASY, EMOTE, TILE, 54086900, new int[]
		{ 17 }, new Item[]
		{ new Item(1059, 1), new Item(1061, 1) }, "Headbang in the mine", "north of Al-kharid.", "Equip Leather Gloves", "and Leather Boots."),
		EMOTE_J_1(EASY, EMOTE, TILE, 42831162, new int[]
		{ 15 }, new Item[]
		{ new Item(1694, 1), new Item(1639, 1), new Item(1103, 1) }, "Dance a jig by the", "entrance to the Fishing Guild,", "equip a sapphire amulet,", "an emerald ring ", "and a bronze chainbody."),
		EMOTE_J_2(MEDIUM, EMOTE, TILE, 54152242, new int[]
		{ 15, 4 }, new Item[]
		{ new Item(3333, 1), new Item(1381, 1) }, "Dance a jig", "under Shantay's Awning.", "Bow or curtsy before you talk to me.", "Equip a pointed blue", "snail helmet, air staff."),
		EMOTE_J_3(EASY, EMOTE, TILE, 45206898, new int[]
		{ 11 }, new Item[]
		{ new Item(4121, 1), new Item(1724, 1), new Item(1353, 1) }, "Jump for joy", "at the beehives.", "Equip iron boots, unholy symbol,", "and a steel hatchet."),
		EMOTE_J_4(MEDIUM, EMOTE, TILE, 42781716, new int[]
		{ 11, 15 }, new Item[]
		{ new Item(9177, 1), new Item(1145, 1), new Item(6324, 1) }, "Jump for joy in Yanille Bank.", "Dance a jig before you talk to me.", "Equip an iron crossbow,", "adamant medium helmet", "and snakeskin chaps."),
		EMOTE_L_1(EASY, EMOTE, TILE, 44912080, new int[]
		{ 12 }, new Item[]
		{ new Item(1167, 1), new Item(1725, 1), new Item(1323, 1) }, "Laugh at the crossroads", "south of Sinclair Mansion.", "Equip a leather cowl,", "amulet of strength", "and iron scimitar."),
		EMOTE_L_2(HARD, EMOTE, TILE, 45780567, new int[]
		{ 12 }, new Item[]
		//{ new Item(1163, 1), new Item(2493, 1), new Item(1393, 1) }, "Laugh in the Jokul's tent", "in the mountain camp.", "Beware of double agents!", "Equip a rune full helm, blue", "d'hide chaps and a fire battlestaff."),
		//EMOTE_P_1(HARD, EMOTE, TILE, 59231649, new int[]
		//{ 20 }, new Item[]
		//{}, "Panic in the heart of", "the Haunted Woods.", "Beware of double agents!", "Have no items equipped when you do."),
		//{ 20 }, new Item[]
		{ new Item(1075, 1), new Item(1269, 1), new Item(1141, 1) }, "Panic in the limestone mine.", "Equip bronze platelegs,", "steel pickaxe", "and steel helm."),
		EMOTE_P_3(HARD, EMOTE, TILE, 46615979, new int[]
		{ 20 }, new Item[]
		{ new Item(1071, 1), new Item(2570, 1), new Item(1359, 1) }, "Panic by the pilot", "on White Wolf Mountain.", "Beware of double agents!", "Equip mithril platelegs,", "a ring of life,", "and a rune hatchet."),
		EMOTE_P_4(EASY, EMOTE, TILE, 43846754, new int[]
		{ 20 }, new Item[] {}, "Panic on the pier where you", "catch the Fishing Trawler.", "Have nothing equipped at all", "when you do.")
		, EMOTE_P_5(EASY, EMOTE, TILE,57413112, new int[] {20, 7}, new Item[] {new Item(1085, 1), new Item(851, 1)}, "Panic by the mausoleum", "in Morytania. ", " Wave before you speak to me.", "Equip a mithril plateskirt,", "a maple longbow and no boots.")
		,
		EMOTE_S_1(HARD, EMOTE, TILE, 47844442, new int[]
		{ 23 }, new Item[]
		{ new Item(1643, 1), new Item(1731, 1) }, "Salute in the banana plantation.", "Beware of double agents!", "Equip a diamond ring,", "amulet of power", "and nothing on your chest and legs."),
		EMOTE_S_2(EASY, EMOTE, TILE, 48745637, new int[]
		{ 8 }, new Item[]
		{ new Item(1654, 1), new Item(1635, 1), new Item(1237, 1) }, "Shrug in the mine near Rimmington,", "Equip a gold necklace,", "a gold ring and a bronze spear."),
		EMOTE_S_3(HARD, EMOTE, TILE, 53087770, new int[]
		{ 8 }, new Item[]
		{ new Item(1075, 1), new Item(1115, 1), new Item(2487, 1) }, "Shrug in the Zamorak temple", "found in the Eastern Wilderness.", "Beware of double agents!", "Equip bronze platelegs,", "an iron platebody,", "and blue d'hide vambraces."),
		EMOTE_T_1(EASY, EMOTE, TILE, 51776739, new int[]
		{ 6 }, new Item[]
		{ new Item(1656, 1), new Item(843, 1) }, "Think in the middle of the", "wheat field by the Lumbridge mill.", "Equip a sapphire necklace", "and an oak shortbow. ")
		//, EMOTE_T_2(MEDIUM,EMOTE, TILE, -1, new int[] {6,16}, new Item[] {new Item(1109,1), new Item(1099,1), new Item(1698,1)}, "Think under the lens in the Observatory.", "Twirl before you talk to me.", "Equip a mithril chainbody,", "green dragonhide chaps", "and ruby amulet. ")
		,
		EMOTE_T_3(EASY, EMOTE, TILE, 48794823, new int[]
		{ 16 }, new Item[]
		{ new Item(1637, 1), new Item(2466, 1), new Item(1095, 1) }, "Twirl at the crossroads", "north of Rimmington.", "Equip a Sapphire ring,", "yellow flowers", "and leather chaps."),
		EMOTE_T_4(EASY, EMOTE, TILE, 50629897, new int[]
		{ 16 }, new Item[]
		{ new Item(1115, 1), new Item(1097, 1), new Item(1155, 1) }, "Twirl in Draynor Manor", "by the fountain.", "Equip an iron platebody,", "studded chaps", "and a bronze full helm. "),
		EMOTE_T_5(MEDIUM, EMOTE, TILE, 50892125, new int[]
		{ 16, 23 }, new Item[]
		{ new Item(1349, 1), new Item(1193, 1), new Item(1159, 1) }, "Twirl on the bridge", "by Gunnarsgrunn (or Barbarian Village).", "Salute before you talk to me.", "Equip an iron hatchet,", "steel kiteshield", "and mithril full helm."),
		EMOTE_W_1(EASY, EMOTE, TILE, 49138733, new int[]
		{ 7 }, new Item[]
		{ new Item(1635, 1), new Item(1095, 1), new Item(1424, 1) }, "Wave on Mudskipper Point. Equip a gold ring, leather chaps and a steel mace. "),
		EMOTE_W_2(EASY, EMOTE, TILE, 54168995, new int[]
		{ 7 }, new Item[]
		{ new Item(1095, 1), new Item(1351, 1), new Item(1131, 1) }, "Wave along the south", "fence of the lumberyard.", "Equip leather chaps,", "bronze hatchet", "and a hardleather body. "),
		EMOTE_Y_1(MEDIUM, EMOTE, TILE, 40012822, new int[]
		{ 13, 8 }, new Item[]
		{ new Item(1698, 1), new Item(1329, 1), new Item(1175, 1) }, "Yawn in the Castle Wars lobby.", "Shrug before you talk to me.", "Equip a ruby amulet,", "mithril scimitar", "and an iron sq shield. "),
		EMOTE_Y_2(EASY, EMOTE, TILE, 50482355, new int[]
		{ 13 }, new Item[]
		{ new Item(1191, 1), new Item(1295, 1), new Item(1097, 1) }, "Yawn in Draynor Marketplace.", "Equip an iron kiteshield,", "steel longsword", "and studded leather chaps."),
		EMOTE_Y_3(EASY, EMOTE, TILE, 52596135, new int[]
		{ 13 }, new Item[]
		{ new Item(1718, 1), new Item(1063, 1), new Item(1335, 1) }, "Yawn in Varrock Palace library.", "Equip a holy symbol,", "leather vambraces", "and an iron warhammer."),
		EMOTE_Y_4(HARD, EMOTE, TILE, 49581685, new int[]
		{ 13 }, new Item[]
		{ new Item(1175, 1), new Item(2487, 1), new Item(1267, 1) }, "Yawn in the rogues' general store.", "Beware of double agents!", "Equip an iron square shield,", "blue dragonhide vambraces,", "and an iron pickaxe.")

		;

		private final int level, type, idType, id;
	...
	{ new Item(995, 20000) //coins
		...
		new Item(3828),
		new Item(3829),
		new Item(3830) //saradomin pages
		,
		new Item(3831),
		new Item(3832),
		new Item(3833),
		new Item(3834) //zamorak pages
		,
		new Item(3835),
		new Item(3836),
		new Item(3837),
		new Item(3838) //guthix pages
		,
		new Item(19600),
		new Item(19601),
		new Item(19602),
		new Item(19603) //bandos pages
		,
		new Item(19604),
		new Item(19605),
		new Item(19606),
		new Item(19607) //armadyl pages
		,
		new Item(19608),
		new Item(19609),
		new Item(19610),
		new Item(19611) //ancient pages
		,
		...
		new Item(1383),
		new Item(1385),
		new Item(1386) //elemental staffs
		,
		new Item(1675),
		new Item(1677),
		new Item(1679),
		new Item(1681),
		new Item(1683) //craftable amulets
		,
		...
		new Item(19478),
		new Item(19479),
		new Item(19480) //teleport scrolls
		,
		new Item(19152, 20),
		new Item(19157, 20),
		new Item(19162, 20) //god arrows
		,
		new Item(19622),
		new Item(19621, 20) //meerkat pouches and fetch casket scroll
		,
		new Item(10280),
		new Item(10282),
		new Item(10284) //composite bows
	...
	{ new Item(2583), new Item(2585), new Item(2587), new Item(2589), new Item(3472) //Trimmed black armour
		,
		new Item(2591),
		new Item(2593),
		new Item(2595),
	...
	private static final Item[] MEDIUM_NORMAL_REWARDS =
	{
		...
		new Item(1183),
		new Item(1199),
		new Item(4823, 100) //adamantite nails
		,
		new Item(1393),
		new Item(1395),
		new Item(1397),
		new Item(1399) //battlestaves
		,
		new Item(1065),
		new Item(1099),
		new Item(1135) //green dhide
		,
		new Item(857),
		new Item(8781, 100) //teak planks
		,
		new Item(373),
		new Item(379) //swordfish + lobs
	};

	private static final Item[] MEDIUM_RARE_REWARDS =
	{ new Item(2599), new Item(2601), new Item(2603), new Item(2605), new Item(3474) //Trimmed adamant armour
		,
		new Item(2607),
		new Item(2609),
		new Item(2611),
		...
		new Item(10418),
		new Item(10436),
		new Item(10438) //purple elegant costume
		,
		...
		new Item(19386),
		new Item(19388),
		new Item(19390) //Vestment robes (Armadyl, Bandos, Zaros, Ancient)
		,
		new Item(13107),
		new Item(13109),
		new Item(13111),
		new Item(13113),
		new Item(13115) //animal masks
		,
	...
	{ new Item(2623), new Item(2625), new Item(2627), new Item(2629), new Item(3477) //Trimmed rune armour
		,
		new Item(2615),
		new Item(2617),
		new Item(2619),
		...
		new Item(10348),
		new Item(10350),
		new Item(10352) //Third-age warrior armour
		,
		new Item(10338),
		new Item(10340),
		new Item(10342),
		new Item(10344) //Third-age mage armour
		,
		new Item(10330),
		new Item(10332),
		new Item(10334),
		new Item(10336) //Third-age ranger armour
		,
		...
		new Item(10370),
		new Item(10372),
		new Item(10374) //Blessed d'hide armour (Zamorak)
		,
		new Item(10384),
		new Item(10386),
		new Item(10388),
		new Item(10390) //Blessed d'hide armour (Saradomin)
		,
		new Item(10376),
		new Item(10378),
		new Item(10380),
		new Item(10382) //Blessed d'hide armour (Guthix)
		,
		new Item(19272),
		new Item(19275),
		new Item(19278),
	...
	private static final Item[] ELITE_NORMAL_REWARDS =
	{ new Item(1305), new Item(1215) //dragon equipment
		,
		new Item(5315),
		new Item(5316) //highend seeds
		,
		new Item(3024),
		new Item(2452) //antifire
		,
	...
	private static final Item[] ELITE_RARE_REWARDS = {
		...
		new Item (41843), new Item (41845), new Item (41847),


		new Item (41861), new Item (41863), new Item (41865), new Item (41867), new Item (41869),
		new Item (41871), new Item (41873), new Item (41875), new Item (41877), new Item (41879),
		new Item (41881),
		...
		new Item(19299),
		new Item(19302),
		new Item(19305) //dragon animal masks
