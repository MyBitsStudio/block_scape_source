...
public class Cooking extends Action {
	...
	public enum Cookables {
		...
		public Item raw;
		private final int lvl, burningLvl, xp;
		private final Item burnt, total;
		private final boolean spitRoast, fireOnly;
	...
	public static double increasedExperience(Player player, double totalXp) {
		...
		if (player.getEquipment().getGlovesId() == 775)
			totalXp *= 1.01;
		if (player.getPerks().isUnlocked(Perks.CHEF.getId()))
			totalXp *= 1.25;
		return totalXp;
	}
	...
	private final Animation FIRE_COOKING = new Animation(897),  RANGE_COOKING = new Animation(897);
	...
	private boolean isBurned(Cookables cook, Player player) {
		...
		if (player.getEquipment().getGlovesId() == 775) {
			if (level >= (cook.getBurningLvl()
					- (cook.getProduct().getId() == 391 ? 0 : 6 + (player.getPerks().isUnlocked(Perks.CHEF.getId()) ? 6 : 0))))
				return false;
		}
	...
	public int processWithDelay(Player player) {
		...
		if ((Utils.random(2) != 0 || (player.getSkills().getLevel(Skills.COOKING) < cook.getBurningLvl())) && isBurned(cook, player)) {
		...
		if (amount > 0) {
			player.sendMessage(
					...
					true);
			return player.getPerks().isUnlocked(Perks.CHEF.getId()) ? 2 : 3;
		}
		return -1;
	}
	...
	public void stop(final Player player) {
		this.setActionDelay(player, player.getPerks().isUnlocked(Perks.CHEF.getId()) ? 2 : 3);
	}
