...
public class Enchanting {
	...
	private static final int AIR_RUNE = 556, WATER_RUNE = 555,
			EARTH_RUNE = 557, FIRE_RUNE = 554, COSMIC_RUNE = 564;
	...
	public enum Types {
		...
		Types(int enchantLevel, int level, int ring, int necklace, int amulet,
				int bracelet) {
		...
		for (Types t : Types.values()) {
			if (enchantLevel == t.enchantLevel) {
				if (player.getSkills().getLevel(Skills.MAGIC) < t.level) {
					player.sendMessage("You need atleast " + t.level + " to enchant "
							+ t.name().toLowerCase() + ".");
					return false;
				}
				if (itemId == base.baseRing) {
					return enchant(player, t.level, base.baseRing, t.ring,
							enchantLevel);
				}
				if (itemId == base.baseNecklace) {
					return enchant(player, t.level, base.baseNecklace,
							t.necklace, enchantLevel);
				}
				if (itemId == base.baseAmulet) {
					return enchant(player, t.level, base.baseAmulet, t.amulet,
							enchantLevel);
				}
				if (itemId == base.baseBracelet) {
					return enchant(player, t.level, base.baseBracelet,
							t.bracelet, enchantLevel);
				}
			}
		}
		return false;
	...
	public static boolean enchant(Player player, int reqLevel, int toEnchant,
			int toMake, int enchantLevel) {
	...
	public static boolean checkRunes(Player player, int level, int enchantLevel) {
		if (enchantLevel == 1) {
			return Magic.checkSpellRequirements(player, level, true,
					COSMIC_RUNE, 1, WATER_RUNE, 1);
		} else if (enchantLevel == 2) {
			return Magic.checkSpellRequirements(player, level, true,
					COSMIC_RUNE, 1, AIR_RUNE, 3);
		} else if (enchantLevel == 3) {
			return Magic.checkSpellRequirements(player, level, true,
					COSMIC_RUNE, 1, FIRE_RUNE, 3);
		} else if (enchantLevel == 4) {
			return Magic.checkSpellRequirements(player, level, true,
					COSMIC_RUNE, 1, EARTH_RUNE, 10);
		} else if (enchantLevel == 5) {
			return Magic.checkSpellRequirements(player, level, true,
					COSMIC_RUNE, 1, EARTH_RUNE, 15, WATER_RUNE, 15);
		} else if (enchantLevel == 6) {
			return Magic.checkSpellRequirements(player, level, true,
					COSMIC_RUNE, 1, FIRE_RUNE, 20, EARTH_RUNE, 20);
		}
		return false;
	}
	...
	public enum Base {
		...
		SAPPHIRE(1, 1637, 1656, 1694, 11072), EMERALD(2, 1639, 1658, 1696,
				11076), RUBY(3, 1641, 1660, 1698, 11085), DIAMOND(4, 1643,
				1662, 1700, 11092), DRAGONSTONE(5, 1645, 1664, 1702, 11115), ONYX(
				6, 6575, 6577, 6581, 11130);

		int level, baseRing, baseNecklace, baseAmulet, baseBracelet;

		Base(int level, int baseRing, int baseNecklace, int baseAmulet,
				int baseBracelet) {
			this.level = level;
	...
	public static boolean canBeEnchanted(Base b, int itemId) {
		return b.baseAmulet == itemId || b.baseNecklace == itemId
				|| b.baseRing == itemId || b.baseBracelet == itemId;
	}
