...
public class LeatherCrafting extends Action {
	...
	public enum LeatherData {
		...
		private final int leatherId, leatherAmount, finalProduct, requiredLevel;
	...
	public static boolean handleItemOnItem(Player player, Item itemUsed, Item usedWith) {
		for (int j : LEATHER) {
			if (itemUsed.getId() == j || usedWith.getId() == j) {
				player.getTemporaryAttributtes().put("leatherType", j);
	...
	private boolean checkAll(Player player) {
		...
		if (!player.getInventory().getItems().containsOne(THREAD) && !player.getPerks().isUnlocked(Perks.CRAFTSMAN.getId())) {
	...
	public int processWithDelay(Player player) {
		...
		player.getSkills().addXp(Skills.CRAFTING,
				data.getExperience() * (player.getPerks().isUnlocked(Perks.CRAFTSMAN.getId()) ? 1.25 : 1));
		Pets.checkSkillingPet(player, 38081);
		player.addItemsMade();
		...
		quantity--;
		if (!player.getPerks().isUnlocked(Perks.CRAFTSMAN.getId())) {
			removeThread--;
			if (removeThread == 0) {
