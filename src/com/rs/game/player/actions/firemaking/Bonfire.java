...
public class Bonfire extends Action {
    ...
    public boolean process(Player player) {
		if (checkAll(player)) {
		    if (Utils.random(player.getPerkManager().thePiromaniac ? 375 : 500) == 0) {
    ...
    public int processWithDelay(Player player) {
		...
		player.getSkills().addXp(Skills.FIREMAKING, Firemaking.increasedExperience(player, log.xp) 
									* (player.getPerkManager().thePiromaniac ? 1.15 : 1));
		player.setNextAnimation(new Animation(16703));
		player.setNextGraphics(new Graphics(log.gfxId));
		player.getPackets().sendGameMessage("You add a log to the fire; logs burned "
		...
		if (count++ == 4 && player.getLastBonfire() == 0) {
		    ...
		    player.sendMessage("<col=00ff00>The bonfire's warmth increases your maximum health by "
    ...
    public boolean start(Player player) {
		if (checkAll(player)) {
		    player.getGlobalPlayerUpdater().setRenderEmote(2498);
		    return true;
		}
		return false;
    }
