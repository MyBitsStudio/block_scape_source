...
public class Fletching extends Action {
	...
	public enum Fletch {
		...
		private final int[] product, level;
		private final int id, selected;
	...
	public boolean process(Player player) {
		...
		if ((!player.getInventory().containsOneItem(fletch.getSelected(), amount)
				&& !((fletch.getSelected() == 1777 || fletch.getSelected() == 314)
						&& player.getPerks().isUnlocked(Perks.FLEDGER.getId()))
	...
	public int processWithDelay(Player player) {
		...
		Pets.checkSkillingPet(player, 38093);
		player.getAchManager().addKeyAmount("fletching", 1);
		...
		return player.getPerks().isUnlocked(Perks.FLEDGER.getId()) ? 0 : 1;
	}
