...
public class TrapAction extends Action {
	...
	public enum Traps {
		...
		/*
		 * obj id, trans id, fail obj id, itemid, itemamount, set emote, remove
		 * emote
		 */
	...
	public enum HunterNPC {
		...
		GRENWALL(Traps.BOX, 77, 1100, new int[] { 28557, 5184, -1, 12539 }),
	...
	public boolean start(Player player) {
		...
		if (currentLevel < levelRequirement) {
			...
			int maxAmount = getMaximumTrap(trap, currentLevel) + (player.getPerks().isUnlocked(Perks.HUNTSMAN.getId()) ? 2 : 0);
			...
			public void run() {
				...
				if (captured != null) {
					...
					for (int i = 3; i < ids.length; i++) {
						...
						player.getResourceGather().submitResourceGatherForStats(new Item(ids[i], 1), captured.getExp() / i, Skills.HUNTER, false);
					}
					player.getSkills().addXp(Skills.HUNTER, captured.getExp());
				}
				Pets.checkSkillingPet(player, 38097);
				player.getAchManager().addKeyAmount("hunter", 1);
				player.addCreaturesCaught();
				player.sendMessage(captured != null
						? "You've caught a " + Utils.formatString(captured.toString())
								+ "; creatures caught: " + Colors.red
								+ Utils.getFormattedNumber(player.getCreaturesCaught()) + "</col>."
