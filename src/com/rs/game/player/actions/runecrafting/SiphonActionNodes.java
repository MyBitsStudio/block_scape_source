...
public class SiphonActionNodes extends Action {
    ...
    private void processNodeDestroy(final Player player) {
    	player.sendMessage("The node you were siphoning from has been depleted of energy.");
    		...
    		if (node.getLife() <= 0) {
    			...
    			if (level < 50 ? Utils.getRandom(2) == 1 : Utils.getRandom(1) == 1) {
    				...
    				if (player.getControlerManager().getControler() != null 
    						&& player.getControlerManager().getControler() instanceof RunespanController) {
    					for (int runeId : nodes.getRuneId())
    						((RunespanController) player.getControlerManager().getControler()).addRunespanPoints(runeId);
    				}
