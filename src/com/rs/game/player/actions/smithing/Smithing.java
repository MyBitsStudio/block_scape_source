...
public class Smithing extends Action {
    ...
    public enum ForgingBar {
		...
		DRACONIC_VISAGE(11286, 90, new Item[] { new Item(11283, 1) },
			new double[] { 2000.0 }, new int[] { 66, 210, 267 }),
		...
		OBSIDIAN(26123, 80, new Item[] { new Item(26124, 1), new Item(26126, 1),
				new Item(26128, 1), new Item(26130, 1), new Item(26132, 1),
				new Item(26134, 1), new Item(26136, 1), new Item(26138, 1),
				new Item(26140, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(-1, 1) }, new double[] { 75, 150, 225, 375 },
				new int[] { 66, 210, 267 }),
    ...
    public static class ForgingInterface {
		...
		private static int getBasedAmount(Item item) {
		    ...
		    if (def.contains("dagger"))
		    	...
		    	return 6;
		    else if (def.contains("knife") || def.contains("full helm") || def.contains("studs") || def.contains("ranger helm")
		    		|| def.contains("mage helm") || def.contains("warrior helm"))
		    	return 7;
		...
		public static String[] getStrings(Player player, ForgingBar bar, int index, int itemId) {
		    ...
		    levelString.append(Utils.formatString(name.replace(barVariableName + " ", "")));
		...
		public static void sendSmithingInterface(Player player, ForgingBar bar, WorldObject object) {
		    ...
		    player.getPackets().sendIComponentText(300, 14,
			    Utils.formatString(bar.toString().toLowerCase()) + "");
    ...
    private double getExperience(Player player) {
    	...
    	double xp = bar.getExperience()[barAmount == 5 ? 3 : barAmount - 1];
		if (player.clickedObject != null && player.clickedObject.getId() == 97270)
			xp *= 1.1;
    ...
    public boolean process(Player player) {
		...
		if (!player.getInventory().containsItem(bar.getBarId(), ForgingInterface.getActualAmount(
		...
		if (player.getSkills().getLevel(Skills.SMITHING) < ForgingInterface.getLevels(bar, index, player)) {
		    player.getDialogueManager().startDialogue("SimpleMessage",
			    "You need a Smithing level of " + ForgingInterface.getLevels(bar, index, player) + " to create this.");
		    return false;
		}
    ...
    public int processWithDelay(Player player) {
		...
		if (Utils.random(100) <= 1 && player.getPlayerVars().hasEfficiencyActivated())
			...
			player.getInventory().deleteItem(bar.getBarId(), 
				ForgingInterface.getActualAmount(bar.getLevel() + 
						ForgingInterface.getFixedAmount(bar, bar.getItems()[index]), bar, bar.getItems()[index].getId()));
		player.getInventory().addItem(bar.getItems()[index].getId(), 
				ForgingInterface.getForgedAmount(bar.getItems()[index].getId()));
		Pets.checkSkillingPet(player, 38107);
		player.getDailyTaskManager().process(bar.getItems()[index].getId());
