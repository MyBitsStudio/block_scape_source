...
public class Summoning implements Serializable {
	...
	public static final class Pouches implements Serializable {
		...
		public static final Pouches PACK_MAMMOTH;
		...
		static {
		    ...
		    HELLCAT = new Pouches("HELLCAT", 77, 9295, 163, 7581, 1,
			    4.9000000000000004D, 435.19999999999999D, 0x3a9800L, 10,
			    12825);
		    PACK_MAMMOTH = new Pouches("PACK_MAMMOTH", 76, 6874, 111, 12093, 96,
				    4.7999999999999998D, 422.19999999999999D, 0x3519c0L, 10,
				    12435);
		    ...
		    ENUM$VALUES = (new Pouches[] { SPIRIT_WOLF, DREADFOWL,
			    ...
			    BLUEDRAGON, GREENDRAGON, BLACKDRAGON, WILYHELLCAT, HELLCAT,PACK_MAMMOTH });
    ...
    public static void listRequirements(final Player player, int itemId, int amount) {
    	...
    	if(pouch == null) {
    		Logger.log("Pouch returned null for data: "+player.getUsername()+" for item ["+itemId+"]");
    		return;
    	}
    ...
    public static void createPouch(final Player player, int itemId, int amount) {
		try {
			...
			for (final Item req : pouch.getItems()) {
				if (!player.getInventory().containsItem(req)) {
					final String name = req.getDefinitions().getName();
					final int a = req.getAmount() * amount;
					final String s = !name.endsWith("s") && a > 1 ? "s" : "";
					player.sendMessage("You still need " + a + " " + name + "" + s + ".");
				}
			}
			...
			for (final Item i : pouch.getItems()) {
				...
				if (i.getId() == 6319 || i.getId() == 10033) {
					if (player.getInventory().getFreeSlots() < amount) {
						...
						if (amount < 1)
							player.sendMessage("Inventory full. To make some room, sell, drop or bank something.", true);
					}
				}
			}
			...
			player.setPouchesMade(player.getPouchesMade() + amount);
			player.sendMessage("You infuse " + (amount == 1 ? "a" : "some") + " "
					+ item.getName().toLowerCase() + (amount == 1 ? "" : "es") + "; pouches made: "
							+ Colors.red+Utils.getFormattedNumber(player.getPouchesMade())+"</col>.");
    ...
    public static double shamanSuit(Player player) {
    	...
    	if (player.getEquipment().getHatId() == 28995
    		...
    		xpBoost *= 1.01;
    	if (player.getEquipment().getHatId() == 32278
    			&& player.getEquipment().getChestId() == 28996
    			&& player.getEquipment().getLegsId() == 28997
    			&& player.getEquipment().getBootsId() == 28999
    			&& player.getEquipment().getGlovesId() == 28998)
