...
public class PickPocketAction extends Action {
	...
	public int processWithDelay(Player player) {
		if(ThroneOfMiscellania.isInArea(player))
			player.getThrone().lostReputation(2.5);
		if (!isSuccesfull(player) && !player.getPerks().isUnlocked(Perks.SLEIGHT.getId())) {
			...
			/*	if(!(manager == null)) {
					if(player.getClanManager().getClan().getClanPerks().hasPerk(ClanPerk.Perks.TIER_3_INCREASED_REVENUE)) {
						player.getClanManager().getClan().getClanRevenue().addClanRevenue(player, item.getAmount() * 3);
					} else if(player.getClanManager().getClan().getClanPerks().hasPerk(ClanPerk.Perks.TIER_2_INCREASED_REVENUE)) {
						player.getClanManager().getClan().getClanRevenue().addClanRevenue(player, item.getAmount() * 2);
					} else if(player.getClanManager().getClan().getClanPerks().hasPerk(ClanPerk.Perks.INCREASED_REVENUE)) {
						player.getClanManager().getClan().getClanRevenue().addClanRevenue(player, item.getAmount());
					}
				}*/
				if (item.getId() == 995)
