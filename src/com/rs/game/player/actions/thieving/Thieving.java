...
public class Thieving {
    ...
    public static void checkGuards(Player player) {
		...
		if (guard != null && !player.getPerks().isUnlocked(Perks.SLEIGHT.getId())) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (!World.containsObjectWithId(object, object.getId()))
							return;
						if (!gaveItems) {
							int itemId = stall.getItem(Utils.getRandom(stall.item.length - 1));
							int amount = Utils.getRandom(stall.getAmount());
							player.getInventory().addItem(itemId, amount);
							player.getSkills().addXp(Skills.THIEVING, stall.getExperience());
							/*if (Taxation.taxItem(player, itemId, amount)) {
								player.getInventory().addItem(itemId, amount);
								player.getSkills().addXp(Skills.THIEVING, stall.getExperience());
							}*/
					    	player.addTimesStolen();
					    	player.sendMessage("You've successfully stolen from this stall; "
					    			+ "times thieved: "+Colors.red+Utils.getFormattedNumber(player.getTimesStolen())+"</col>.", true);
						    gaveItems = true;
						    checkGuards(player);
			
						    ClansManager manager = player.getClanManager();
							/*if(!(manager == null))
								if(player.getClanManager().getClan().getClanPerks().hasPerk(ClanPerk.Perks.TIER_3_INCREASED_REVENUE)) {
									player.getClanManager().getClan().getClanRevenue().addClanRevenue(player, stall.getAmount() * 3);
								} else if(player.getClanManager().getClan().getClanPerks().hasPerk(ClanPerk.Perks.TIER_2_INCREASED_REVENUE)) {
									player.getClanManager().getClan().getClanRevenue().addClanRevenue(player, stall.getAmount() * 2);
								} else if(player.getClanManager().getClan().getClanPerks().hasPerk(ClanPerk.Perks.INCREASED_REVENUE)) {
									player.getClanManager().getClan().getClanRevenue().addClanRevenue(player, stall.getAmount());
								} else */
    ...
    public static boolean pickDoor(Player player, WorldObject object) {
		...
		if (Math.round(ratio * thievingLevel) < (player.getAttackedByDelay() > 0 ? 50 : 40)
			/ player.getAuraManager().getThievingAccurayMultiplier() && !player.getPerks().isUnlocked(Perks.SLEIGHT.getId())) {
