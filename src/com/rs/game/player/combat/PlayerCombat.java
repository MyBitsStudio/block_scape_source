...
public class PlayerCombat extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if (player.getPerks().isUnlocked(Perks.BREATH.getId()) && Utils.random(20) == 0) {
			if (spellId < 1 && hasPolyporeStaff(player))
				spellId = 2672;
			...
			if (spellId < 1 && hasStaffOfSliske(player))
				spellId = 2672;
			if (spellId < 1 && hasStaffOfCorrupt(player))
				spellId = 2672;
		...
		if (target instanceof BallakDefs) { // Fixed ballak Safe Spotting
			maxDistance = 0;
		}
	...
	private int getRangeCombatDelay(int shieldId, int weaponId, int attackStyle) {
		...
		if (weaponId != -1) {
			...
			if (weaponName.contains("shortbow") || weaponName.contains("karil") || weaponName.contains("zaryte")
					...
					|| weaponName.contains("winds of waiko"))
				delay = 3;
			else if (weaponName.contains("crossbow") && !shieldName.contains("crossbow")
					|| weaponName.contains("crossbow") && !shieldName.contains("repriser"))
				delay = 4;
			else if (weaponName.contains("crossbow") && shieldName.contains("crossbow")
					|| weaponName.contains("crossbow") && shieldName.contains("repriser"))
				delay = 1;
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (spellId == 2474) { // hellfire
			...
			// World.sendProjectile(shooter, startTile, receiver, gfxId, startHeight,
			// endHeight, speed, delay, curve, startDistanceOffset);
			return 3;
		}
		...
		if (spellId == 2672) { // breath with staff 17958
			...
			attackTarget(getMultiAttackTargets(player), new MultiAttack() {
				...
				public boolean attack() {
					...
					mage_hit_gfx = 5025;
					magic_sound = 161;
					// int damage = getRandomMagicMaxHit(player, 319);
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 56: // slayer dart
				...
				for (int[] level : levels) {
					if (player.getSkills().getLevel(Skills.MAGIC) >= level[0]
							&& player.getSkills().getLevel(Skills.MAGIC) < level[1])
						delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, level[2])));
				}
			...
			case 45: // fire bolt
				...
				if (target instanceof NPC) {
					...
					if (n.getId() == 9463) // ice verm
						damage *= 1.5;
					if (n.getId() == 14301 || n.getId() == 51) // Glacors
						baseDamage *= 2;
				}
				delayMagicHit(2, getMagicHit(player, damage));
			...
			case 52: // water blast
				...
				base_mage_xp = 31.5;
				magic_sound = 208;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 312)));
			...
			case 58: // earth blast
				...
				base_mage_xp = 31.5;
				magic_sound = 129;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 320)));
			...
			case 63: // fire blast
				...
				base_mage_xp = 34.5;
				magic_sound = 156;
				damage = getRandomMagicMaxHit(player, 355);
				if (target instanceof NPC) {
					NPC n = (NPC) target;
					if (n.getId() == 9463) // ice verm
						damage *= 1.5;
			...
			case 70: // air wave
				...
				base_mage_xp = 36;
				magic_sound = 223;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 350)));
				World.sendProjectile(player, target, 2699, 18, 18, 50, 50, 0, 0);
				return getMagicAttackDelay(player, weaponId);
			case 73: // water wave
			...
			case 77: // earth wave
				...
				base_mage_xp = 42.5;
				magic_sound = 135;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 390)));
			...
			case 80: // fire wave
				...
				base_mage_xp = 42.5;
				magic_sound = 163;
				damage = getRandomMagicMaxHit(player, 420);
				if (target instanceof NPC) {
					NPC n = (NPC) target;
					if (n.getId() == 9463) // ice verm
						damage *= 1.5;
			...
			case 87:// water surge
				...
				base_mage_xp = 80;
				magic_sound = 214;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 490)));
			...
			case 91:// fire surge
				...
				base_mage_xp = 80;
				magic_sound = 163;
				damage = getRandomMagicMaxHit(player, 600);
			...
			case 32:// Shadow Rush
				...
				magic_sound = 177;
				reduceAttack = true;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 276)));
			...
			case 37: // Miasmic blitz
				...
				mage_hit_gfx = 1851;
				base_mage_xp = 48;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 408)));
			...
			case 38: // Miasmic burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						mage_hit_gfx = 1849;
						base_mage_xp = 42;
						int damage = getRandomMagicMaxHit(player, 364);
						delayMagicHit(2, getMagicHit(player, damage));
						if (target.getTemporaryAttributtes().get("miasmic_immunity") != Boolean.TRUE) {
							if (target instanceof Player) {
								((Player) target).getPackets().sendGameMessage("You feel slowed down.");
			...
			case 39: // Miasmic barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						mage_hit_gfx = 1854;
						base_mage_xp = 54;
						int damage = getRandomMagicMaxHit(player, 452);
						delayMagicHit(2, getMagicHit(player, damage));
						if (target.getTemporaryAttributtes().get("miasmic_immunity") != Boolean.TRUE) {
							if (target instanceof Player) {
								((Player) target).getPackets().sendGameMessage("You feel slowed down.");
			...
			case 24:// Blood rush
				...
				magic_sound = 174;
				blood_spell = true;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 287)));
			...
			case 20:// Ice rush
				...
				freeze_time = 5000;
				magic_sound = 173;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 298)));
			...
			case 30:// Smoke burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						max_poison_hit = 20;
						magic_sound = 180;
						int damage = getRandomMagicMaxHit(player, 309);
						delayMagicHit(2, getMagicHit(player, damage));
			...
			case 26:// Blood burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 39;
						blood_spell = true;
						int damage = getRandomMagicMaxHit(player, 322);
						delayMagicHit(2, getMagicHit(player, damage));
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 22:// Ice burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						freeze_time = 15000;
						magic_sound = 169;
						int damage = getRandomMagicMaxHit(player, 342);
			...
			case 29:// Smoke Blitz
				...
				magic_sound = 184;
				max_poison_hit = 40;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 353)));
				World.sendProjectile(player, target, 386, 18, 18, 50, 50, 0, 0);
				return 4;
			case 33:// Shadow Blitz
				...
				magic_sound = 180;
				reduceAttack = true;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 364)));
				World.sendProjectile(player, target, 380, 18, 18, 50, 50, 0, 0);
				return 4;
			case 25:// Blood Blitz
				player.setNextAnimation(new Animation(1978));
				...
				magic_sound = 105;
				blood_spell = true;
				int damage = getRandomMagicMaxHit(player, 375);
				delayMagicHit(2, getMagicHit(player, damage));
				World.sendProjectile(player, target, 374, 18, 18, 50, 50, 0, 0);
				return 4;
			case 21:// Ice Blitz
				...
				freeze_time = 15000;
				magic_sound = 169;
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 386)));
			...
			case 35:// shadow barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						magic_sound = 178;
						reduceAttack = true;
						int damage = getRandomMagicMaxHit(player, 408);
						delayMagicHit(2, getMagicHit(player, damage));
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 27:// blood barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						magic_sound = 107;
						blood_spell = true;
						int damage = getRandomMagicMaxHit(player, 419);
						delayMagicHit(2, getMagicHit(player, damage));
						World.sendProjectile(player, target, 390, 18, 18, 50, 50, 0, 0);
						if (!nextTarget) {
							if (damage == -1)
			...
			case 23: // ice barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 52;
						int damage = getRandomMagicMaxHit(player, 450);
	...
	private int getMagicMaxHit(Player player, int baseDamage) {
		...
		if (target instanceof NPC) {
			if (Dungeoneering.hasPrimalHelm(player) || Dungeoneering.hasPrimalChest(player)
					|| Dungeoneering.hasPrimalLegs(player) || Dungeoneering.hasStarFire(player)) {// TODO
				if (Dungeoneering.DragonKinArea(player) && player.inDungeoneering) {
					EA = Math.floor(EA * 1.5);
					// player.sm(Colors.red + "Damage increased because you are using starfire on
					// Dragonkin");
				}
			}
		}
		...
		if (player.getEquipment().getAmuletId() == 31872)
			accuracy += 0.03;
		if (player.getPetManager().isAttackingPet()) {
			player.isAtWild();
		}
		max_hit = baseDamage;
		double boost = 1
	...
	private int rangeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			/*
			 * if (getSpecialAmmount(weaponId) >= 35 && target instanceof CommanderZilyana)
			...
			case 24456: // decimation
				...
				target.applyHit(new Hit(player, Utils.random(Utils.random(-100), Utils.random(100 - 500)),
						HitLook.RANGE_DAMAGE));
				// addAttackedByDelay(player, 2000);
				break;
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponName.contains("dart")
					...
					checkSwiftGlovesEffect(player, 1, attackStyle, weaponId, hit, projectileGfx, 41, 32, 15, 0);
					...
					if (ammoId != -1 && Utils.getRandom(10) == 5) {
						...
						damage = getRandomMaxHit(player, weaponId, attackStyle, true);
						checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, damage, projectileGfx, 38, 32,
								5, 0);
					}
					World.sendProjectile(player, target, projectileGfx, 38, 36, 41, 32, 5, 0);
					delayHit(2, weaponId, attackStyle, getSpecialRangeHit(player, damage));
					if (weaponId != 4740)
						dropAmmo(player);
					else
					...
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, hit, projectileGfx, 39, 48, 0, 0);
					...
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, hit, 2119, 39, 48, 0, 0);
					...
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, hit, projectileGfx, 38, 50, 10,
							100);
	...
	public Hit getSpecialRangeHit(Player player, int damage) {
	...
	private void checkSwiftGlovesEffect(Player player, int hitDelay, int attackStyle, int weaponId, int hit, int gfxId,
										int startHeight, int delay, int curve, int startDistanceOffset) {
		if (hit > 0 && Utils.getRandom(30) < 29)
			return;
		Item gloves = player.getEquipment().getItem(Equipment.SLOT_HANDS);
		if (gloves == null || !gloves.getDefinitions().getName().contains("Swift glove"))
			return;
		World.sendProjectile(player, target, gfxId, startHeight - 5, 36 - 5, 41, delay,
				Math.max(curve - 5, 0), startDistanceOffset);
		...
		if (quantity == -2) {
			...
			if (weaponId != -1) {
				...
				if ((quantity == -3 && Utils.getRandom(10) < 2) || (quantity != -3 && Utils.getRandom(3) > 0)) {
					if (Combat.hasAvas(player) || player.getPerks().isUnlocked(Perks.AVA.getId())) {
	...
	private int meleeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 40707:
			case 42067:
				player.setNextAnimationForce(new Animation(18578)); // (11989));
				// target.setNextGraphics(new Graphics(5513));
				delayHit(1, weaponId, attackStyle,
				...
				delayHit(3, weaponId, attackStyle,
						getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.15, true)));
				// delayNormalHit(weaponId, attackStyle,
				// getMagicHit(player, getRandomMaxHit(player, weaponId, attackStyle, false,
				// true, 2, true)),
			...
			case 13904: // (new Animation(10505));
			case 39117: // (new Graphics(1840));
				player.setNextAnimationForce(new Animation(10505));
				...
				if (target instanceof Player) {
					Player targetPlayer = ((Player) target);
					if (targetPlayer.getSkills().drainLevel(Skills.DEFENCE, damage1 / 30) > 0) {
						break;
					}
				}
				break;
			...
			case 18786:
				...
				int korasiDamage = getMaxHit(player, weaponId, attackStyle, false, 1);
			...
			if (player.getPerks().isUnlocked(Perks.ANNIHILATOR.getId()) && Utils.random(18) == 0) {
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects,
		...
		max_hit = getMaxHit(player, weaponId, attackStyle, ranging, specMultiplier);
		if (defenceAffects) {
			...
			if (player.getSlayerManager() != null) {
				if (target instanceof NPC) {
					if (Slayer.hasSpecialSlayerHelmet(player)) {// TODO
						if (player.getSlayerManager().isValidTask(((NPC) target).getName()))
							att *= 1.3;
					}
				}
			...
			if (player.getSlayerManager() != null) {
				if (target instanceof NPC) {
					if (Slayer.hasSlayerKhopesh(player)) {// TODO
						if (player.getSlayerManager().isValidTask(((NPC) target).getName()))
							att *= 1.6;
					}
				}
			...
			if (player.getSlayerManager() != null) {
				if (target instanceof NPC) {
					if (Slayer.hasSlayerTwang(player)) {// TODO
						if (player.getSlayerManager().isValidTask(((NPC) target).getName()))
							att *= 1.5;
					}
				}
			...
			if (player.getSlayerManager() != null) {
				if (target instanceof NPC) {
					if (Slayer.hasSlayerOffTwang(player)) {// TODO
						if (player.getSlayerManager().isValidTask(((NPC) target).getName()))
							att *= 1.3;
					}
				}
			...
			if (player.getSlayerManager() != null) {
				if (target instanceof NPC) {
					if (Slayer.hasSlayerRaptor(player)) {// TODO
						if (player.getSlayerManager().isValidTask(((NPC) target).getName()))
							att *= 1.125;
					}
				}
	...
	private int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging,
		...
		if (!ranging) {
			...
			if (target instanceof NPC) {
				if (Slayer.hasSlayerKhopesh(player) || Slayer.hasoffhandKhopesh(player)) {// TODO
					if (player.getSlayerManager().isValidTask(((NPC) target).getName())) {
	...
	public static boolean fullVoidEquipped(Player player, int... helmid) {
		...
		boolean hasTorso = (torsoId == 8839 || torsoId == 10611 || torsoId == 19785 || torsoId == 19787 || torsoId == 19789);
		if (!hasTorso) {
			if (hasDeflector)
				hasDeflector = false;
			else
	...
	private void delayHit(int delay, final int weaponId, final int attackStyle, final Hit... hits) {
		...
		for (Hit hit : hits) {
			...
			damage = Math.min(hit.getDamage(), target.getHitpoints());
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				for (Hit hit : hits) {
					...
					// lifesteal perk
					if (player.getPerks().isUnlocked(Perks.DOMINATOR.getId()) && Utils.random(5) == 0
							&& !player.getUsername().contains("red_arrow")) {
						if (hit.getDamage() > 0) {
							int damage = hit.getDamage();
							player.applyHit(new Hit(null, (int) (damage * 0.05), HitLook.HEALED_DAMAGE));
					...
					// TODO stun perk
					if (player.getPerks().isUnlocked(Perks.STAGGER.getId()) && Utils.random(15) == 0) {
					...
					if (player.getPet() != null && player.getPerks().isUnlocked(Perks.COMPANION.getId())) {
	...
	public static int getWeaponAttackEmote(int weaponId, int attackStyle, Player player) {
		...
		String weaponName = ItemDefinitions.getItemDefinitions(weaponId).getName().toLowerCase();
		final boolean hasOffhand = new Item(player.getEquipment().getShieldId()).getName().toLowerCase().contains(
				"off-hand") || new Item(player.getEquipment().getShieldId()).getName().toLowerCase().contains("offhand")
	...
	public static int getAttackSpeed(Player player) {
		...
		if (name.contains("dagger") || name.contains(" sword") || name.contains("scimitar")
				|| name.contains("zaros godsword") || name.contains("godsword") || name.contains("rapier")
				|| name.contains("blade") || name.contains("maul") || name.contains("whip")
				|| name.contains("starfire sword") || name.contains("thalassia's revenge") || name.contains("2h")
				|| name.contains("spear") || name.contains("dominion"))
			return 2;
		if ((name.contains("scythe") || name.contains("lance") || name.contains("retro")))
			return 3;
		final boolean hasOffhand = new Item(player.getEquipment().getShieldId()).getName().contains("Off-hand");
	...
	private boolean checkAll(Player player) {
		...
		maxDistance = player.getEquipment().getWeaponId() == 33590 || isRanging != 0
				|| player.getCombatDefinitions().getSpellId() > 0 || hasNoxiousStaff(player) || hasCywirWand(player)
				|| hasStaffOfSliske(player) || hasSeismicWand(player) || hasPraesulWand(player)
	...
	private boolean canAttackNPC(Player player, NPC n) {
		...
		/*
		 * if (World.isAtKuradalsDungeon(player)) { if
		 * (player.getSlayerManager().isValidTask(n.getName())) {
		 * player.sendMessage("You can only attack this monster when on a Slayer task."
		 * ); return false; } } if (npcName.equalsIgnoreCase("edimmu")) { if
		 * (player.getSlayerManager().isValidTask(n.getName())) {
		 * player.sendMessage("You can only attack this monster when on a Slayer task."
		 * ); return false; } }
		 */
	...
	public static boolean hasMeleeDistanceWeapon(final Player player) {
	...
	public static int isRanging(Player player) {
		...
		if (weaponId == -1)
			return 0;
		if (hasPolyporeStaff(player) || (hasNoxiousStaff(player)
				|| (hasStaffOfSliske(player) || hasPraesulWand(player) || hasSeismicWand(player)))) {
			return 2;
		}
		...
		if (name.contains("knife") || name.contains("dart") || name.contains("javelin")
				...
				|| name.equalsIgnoreCase("zaryte bow") || name.equalsIgnoreCase("sagittarian longbow")
				|| name.equalsIgnoreCase("sagittarian shortbow") || name.equalsIgnoreCase("boogie bow")
				|| name.equalsIgnoreCase("Winds of Waiko") || name.equalsIgnoreCase("Starfire bow")
				|| name.equalsIgnoreCase("strykebow") || name.contains("chinchompa") || name.contains("decimation")
				|| name.contains("bolas") || name.contains("starfire bow") || name.contains("chakram")
				|| name.contains("glaive") || name.contains("seren godbow")
				|| name.contains("blightbound crossbow"))
			return 2;
		switch (weaponId) {
		case 16867:
		case 15775:
		...
		case 37125: // twang c'bow
		case 37126: // twang c'bow
		case 28416:
		case 28418:
		case 28439:
		case 33318:
	...
	private static int getMagicAttackGraphics(int weaponId, int spellId) {
		String weaponName = ItemDefinitions.getItemDefinitions(weaponId).getName().toLowerCase();
	...
	private static boolean hasStaffOfCorrupt(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
		return weaponId == 66666;
	}
