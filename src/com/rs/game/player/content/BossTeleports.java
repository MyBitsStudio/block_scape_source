...
public class BossTeleports {
	...
	public static void sendInterface(Player player) {
		...
		player.getPackets().sendIComponentText(813, 56, Colors.green + "Barrel Chest");
	...
	public static void handleInterface(Player player, int componentId) {
		...
		if (componentId == 56)
			Magic.vineTeleport(player, new WorldTile(3873, 4700, 0)); // barrelchest
		if (componentId == 60)
		...
		if (componentId == 51) { // greg
			...
			Instance instance1 = null;
			for (int i = 0; i < World.getInstances().size(); i++) {
				if (World.getInstances().get(i) instanceof GregorovicInstance) {
					instance1 = World.getInstances().get(i);
				}
			}
			if (instance1 != null) {
				...
				instance1 = new GregorovicInstance(player, 60, 33, 5, -1, 3, false);
				instance1.constructInstance();
			}**/
