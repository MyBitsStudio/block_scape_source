...
public class CarrierTravel {
	...
	private static final int[] REGIONS = new int[] { 8496, 14646, 11061, 11823, 11825, 11058, 10545, 12081, 14637,
			10284 };
	...
	public enum Carrier {
		...
		Port_Phasmatys(new int[] { 3200, -1, 3500, -1, 2900, 2900, 4100, 1300, -1, 2800 },
		...
		Port_Khazard(new int[] { 3200, 4100, 1600, 1600, 1600, 1600, -1, 1280, 1025, 5000 },
	...
	public static boolean sendCarrier(final Player player, final Carrier ship, int shipIndex, boolean returning) {
		if (player.getTemporaryAttributtes().get("using_carrier") != null)
			return false;
		if ((ship.ordinal() == Carrier.Karamja.ordinal())
				&& player.getInventory().containsItem(PiratesTreasure.KARAMJA_RUM, 1)) {
		...
		if (isFare) {
			...
			player.getPackets().sendConfig(75, configValue);
		}
	...
	public static void handleCharterOptions(Player player, int componentId) {
		...
		player.getDialogueManager().startDialogue("QuickCharter", ship,
				ship.getCosts()[getCostIndex(player.getRegionId())]);
	}
