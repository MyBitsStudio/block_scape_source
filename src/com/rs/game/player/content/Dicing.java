...
public class Dicing {
    ...
    public static void friendsRoll(final Player player, final int itemId, int graphic, final int lowest, final int highest) {
		...
		if (!player.getCurrentFriendChatOwner().equalsIgnoreCase("dicing")
				&& !player.isDeveloper()) {
		    player.sendMessage("You need to be in the 'Dicing' Friends chat to roll a dice.");
		    return;
		}
		player.lock(2);
		...
		WorldTasksManager.schedule(new WorldTask() {
		    ...
		    public void run() {
		    	chat.sendDiceMessage(player, "Friends Chat channel-mate <col=db3535>" + player.getDisplayName()
					+ "</col> rolled <col=db3535>" + getRandom(lowest, highest) + "</col> on " + diceText(itemId) + " die.");
		    	player.unlock();
		    }
		}, 1);
    }
