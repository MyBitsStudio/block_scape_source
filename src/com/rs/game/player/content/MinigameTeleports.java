...
import com.rs.game.world.controllers.RunespanController;
...
public class MinigameTeleports {
	...
	public static void sendInterface(Player player) {
		...
		player.getPackets().sendIComponentText(813, 52, Colors.white + "Barrows");
		...
		player.getPackets().sendIComponentText(813, 50, Colors.white + "Rise Of The Six");
		player.getPackets().sendIComponentText(813, 51, Colors.white + "Tusken invasion");
	...
	public static void handleInterface(Player player, int componentId) {
		...
		if (componentId == 50) {// ROTS
			if (player.isAtWild()) {
				player.getPackets().sendGameMessage("A magical force is blocking you from teleporting.");
				return;
			}
			Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3542, 3307, 0));
		}
		if (componentId == 51) { // tusken invasion
			if (player.isAtWild()) {
				player.getPackets().sendGameMessage("A magical force is blocking you from teleporting.");
				return;
			}
		...
		// if (componentId == 51) //Maczab invasion
		// player.getControlerManager().startControler("tuskenraid");
		// if (componentId == 37) //Rise of the six 2.0
		// Magic.vineTeleport(player, new WorldTile(3542, 3307, 0));
