	...
	private enum Effects {
		...
		SUMMONING_POT(Skills.SUMMONING) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				...
				return Math.min(actualLevel + restore, realLevel);
			}
		...
		ANTIPOISON() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 86000);
		...
		SUPER_ANTIPOISON() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 346000);
				player.sendMessage("You are now immune to all kinds of poison.");
		...
		ANTI_FIRE() {
			...
			public void extra(final Player player) {
				player.addFireImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 360000);
				...
				}, (player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 500, 100);
			}
		},
		SUPER_ANTI_FIRE() {
			@Override
			public void extra(final Player player) {
				player.addSuperAntiFire((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 360000);
		...
		OVERLOAD() {
			...
			@Override
			public void extra(final Player player) {
				player.setOverloadDelay((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 501);
		...
		PRAYER_RENEWAL() {
			@Override
			public void extra(Player player) {
				player.setPrayerRenewalDelay((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 501);
		...
		GRAND_STRENGTH(Skills.STRENGTH) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				final int level = Math.min(actualLevel, realLevel);
		...
		SUPREME_OVERLOAD(Skills.ATTACK, Skills.DEFENCE, Skills.STRENGTH, Skills.RANGE, Skills.MAGIC) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				final int level = Math.min(actualLevel, realLevel);
				return (int) (level + 6 + (realLevel * 0.18));
			}
		},
		...
		WEAK_STAT_CURE_POTION() {
			@Override
			public void extra(Player player) {
				player.addFireImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 500);
				player.addPoisonImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 500);
				if (player.getPerks().isUnlocked(Perks.HERBIVORE.getId()))
		...
		REGULAR_STAT_CURE_POTION() {
			@Override
			public void extra(Player player) {
				player.addSuperAntiFire((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 1000);
				player.addPoisonImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 1000);
				if (player.getPerks().isUnlocked(Perks.HERBIVORE.getId()))
					player.getPackets().sendGameMessage(
		...
		STRONG_STAT_CURE_POTION() {
			@Override
			public void extra(Player player) {
				player.addSuperAntiFire((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 2000);
				player.addPoisonImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 2000);
				if (player.getPerks().isUnlocked(Perks.HERBIVORE.getId()))
					player.getPackets().sendGameMessage(
							"Your poison immunity and super anti fire immunity time increased by double for having herbivore perk.");
			}
		...
		STRONG_REJUVINATION_POTION(Skills.SUMMONING, Skills.PRAYER) {
			...
			public void extra(Player player) {
				player.getPrayer()
						.restorePrayer(100 + (player.getSkills().getLevelForXp(Skills.PRAYER) * 10) * 22);
			}
		},
		...
		ANTIPOISON_PLUS() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 900);
				if (player.getPerks().isUnlocked(Perks.HERBIVORE.getId()))
					player.getPackets().sendGameMessage(
		...
		ANTIPOISON_DOUBLE_PLUS() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerks().isUnlocked(Perks.HERBIVORE.getId()) ? 2 : 1) * 1200);
				if (player.getPerks().isUnlocked(Perks.HERBIVORE.getId()))
					player.getPackets().sendGameMessage(
							"Your poison immunity time increased by double for having herbivore perk.");
			}
		...
		TANKARD("You quaff the beer. You feel slightly reinvigorated... but very dizzy too.", Skills.ATTACK,
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == Skills.STRENGTH) {
					...
					return actualLevel > 9 ? actualLevel - 9 : actualLevel >> 9;
				}
			}
		...
		KEG_OF_BEER("You chug the keg. You feel reinvigorated... ...but extremely drunk, too.", Skills.ATTACK,
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == Skills.STRENGTH) {
					...
					return actualLevel > 40 ? actualLevel - 40 : actualLevel >> 8;
				}
			}
		...
		GROG("You drink a glass of grog.", Skills.ATTACK, Skills.STRENGTH) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == Skills.STRENGTH) {
					...
					return actualLevel > 6 ? actualLevel - 6 : actualLevel >> 6;
				}
			}
		...
		CIDER("You drink a glass of cider.", Skills.ATTACK, Skills.STRENGTH, Skills.FARMING) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == Skills.FARMING) {
					...
					return actualLevel > 2 ? actualLevel - 2 : actualLevel >> 2;
				}
			}
		...
		DRAGON_BITTER("You drink a glass of Dragon Bitter.", Skills.ATTACK, Skills.STRENGTH) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == Skills.STRENGTH) {
					...
					return actualLevel > 1 ? actualLevel - 1 : actualLevel >> 1;
				}
			}
		...
		RANGERS_AID("You drink a glass of Ranger's Aid.", Skills.RANGE, Skills.DEFENCE) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == Skills.RANGE) {
					...
					return actualLevel > 5 ? actualLevel - 5 : actualLevel >> 5;
				}
			}
		...
		WIZARD_MIND_BOMB("You drink a glass of Wizard's mind bomb.", Skills.ATTACK, Skills.STRENGTH, Skills.MAGIC,
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == Skills.MAGIC) {
					...
					int boost = player.getSkills().getLevelForXp(Skills.MAGIC) >= 50 ? 3 : 2;
					int level = Math.min(actualLevel, realLevel);
					return level + boost;
				} else if (skillId == Skills.ATTACK) {
					return actualLevel > 4 ? actualLevel - 4 : actualLevel >> 4;
				} else {
					return actualLevel > 3 ? actualLevel - 3 : actualLevel >> 3;
				}
			}
