...
import com.rs.game.world.controllers.CitadelControler;
...
public class ClansManager {
	...
	public static int interfaceSelected;
	...
	public static void viewClanmateDetails(Player player, ClanMember member) {
		...
		player.getPackets().sendGlobalString(347, Utils.formatString(member.getUsername()));
		player.getPackets().sendRunScriptBlank(4319);
		player.getVarsManager().sendVarBit(2149, 71368704);
	}
	...
	void checkUnlocks() {
		getClan().getClanPerks().getPerkUnlockLevel(player);
		getClan().getClanAchievements().checkAchievements(player);
	...
	public static void leaveClanCompletly(Player player) {
		...
		if(player.getControlerManager().getControler() instanceof CitadelControler)
			player.setNextWorldTile(new WorldTile(2963, 3290, 0));
			manager.getClan().getClanCitadel().removeFromCitadel(player);
		player.lock(1);
		if (manager == null)
			return;
		...
		synchronized (cachedClans) {
			synchronized (this) {
				...
				if (player != null) {
					...
					player.sm("You're no longer part of a clan.");
				}
				if (clan.getMembers().isEmpty()) {
					...
					if (mate.getRank() == Clan.LEADER) {
						...
						if (newLeader != null) {
							...
							generateClanChannelDataBlock();
							refreshClanChannel();
							sendGlobalMessage("<col=7E2217>" + Utils.formatString(newLeader.getUsername()) + " has been appointed as new leader!");
						}
					}
					generateClanSettingsDataBlock();
					refreshClanSettings();
	...
	public static void saveClanmateDetails(Player player) {
		...
		synchronized (manager) {
			if (rank != null && member.getRank() == Clan.LEADER) {
				...
				if (newLeader == null) {
					player.sm("Please select a deputy owner before changing your own rank.");
					return;
				}
				newLeader.setRank(Clan.LEADER);
				manager.clan.setClanLeaderUsername(newLeader);
				manager.sendGlobalMessage("<col=7E2217>" + Utils.formatString(newLeader.getUsername()) + " has been appointed as new leader!");
			}
			if (rank != null) {
				member.setRank(rank);
	...
	public static void getOwners(ClanMember member, Player player) {
		ClansManager manager = player.getClanManager();
		if(manager == null)
			return;
		if (member.getRank() == Clan.LEADER && member.getRank() == Clan.DEPUTY_OWNER) 
			return;
	}

	public static void openClanInformation(Player player) {
		ClansManager manager = player.getClanManager();
		if(player.getClanManager() == null) {
			player.sm("You must be in a clan to do that.");
			return;
		}
		if (manager.hasRankToEditSettings(player)) 
			player.getDialogueManager().startDialogue("ClanOwner");
		else 
			openClanDetails(player);
	}
	...
	public void kickPlayerFromChat(Player player, String username) {
		...
		synchronized (this) {
			...
			if (kicked == null) {
				player.sm("This player is not this channel.");
				return;
			}
			...
			player.sm("You have kicked " + kicked.getUsername() + " from clan chat channel.");
	...
	public boolean isLeader(Player player) {
		return getRank(player) == Clan.LEADER;
	}
	...
	public String getRankByName(Player player) {
		if(getRank(player) == Clan.RECRUIT) {
			return "Clan Recruit";
		} else if(getRank(player) == Clan.ADMIN) {
			return "Clan Administrator";
		} else if(getRank(player) == Clan.DEPUTY_OWNER) {
			return "Clan Deputy Owner";
		} else if(getRank(player) == Clan.LEADER) {
			return "Clan Leader";
		}
		return null;
	}
	...
	public void generateClanSettingsDataBlock() {
		...
		for (ClanMember member : clan.getMembers()) {
			// stream.writeLong(
			stream.writeString(Utils.formatString(member.getUsername()));
	...
	public static void linkClanMember(Player player, String clanName) {
		...
		player.sm("You have joined the clan, so you are now part of " + clanName + ".");
		connectToClan(player, clanName, false);
	}

	public static void createClan(Player player, String clanName) {
		clanName = Utils.formatString(clanName);
		if (player.getClanManager() != null)
			return;
		synchronized (cachedClans) {
			if (SerializableFilesManager.containsClan(clanName)) {
				player.sm("The clan name you tried already exists.");
				return;
			}
		...
		synchronized (cachedClans) {
					...
					if (guest) {
						...
						if (bannedSince != null) {
							if (bannedSince + 3600000 > Utils.currentTimeMillis()) {
								player.sm("You have been banned from this channel.");
								return false;
							}
			...
			synchronized (manager) {
				if (!guest && !manager.isMember(player)) {
					player.sm("You have been kicked from the clan.");
					return false;
				}
				if (guest) {
					if (!manager.clan.isGuestsInChatCanEnter()) {
						player.sm("This clan only allows clanmates to join their channel.");
	...
	public static void openClanDetails(Player player, Player p2, ClansManager manager) {
		...
		player.getPackets().sendIComponentText(1107, 32, "Clan level: ");
		player.getPackets().sendIComponentText(1107, 36, "" + player.getClanManager().getClan().getClanLevel().getClanLevel());
		player.getPackets().sendIComponentText(1107, 34, "Clan exp: ");
		player.getPackets().sendIComponentText(1107, 38, "" + (player.getClanManager().getClan().getClanLevel().getClanXp() >= ClanLevel.cXp[23] ? "Experience Capped" : + player.getClanManager().getClan().getClanLevel().getClanXp() + "/" + ClanLevel.cXp[player.getClanManager().getClan().getClanLevel().getClanLevel() - 1]));
		player.getPackets().sendIComponentText(1107, 60, player.getClanName() + "'s Information");
		player.getPackets().sendIComponentText(1107, 87, player.getClanName() + " message:");
		if(manager.clan.getClanMessage() != null)
			player.getPackets().sendIComponentText(1107, 89, player.getClanManager().getClan().getClanMessage());
		if (p2 != null)
	...
	public static void banPlayer(Player player) {
		...
		if (manager.getRank(player) < Clan.ADMIN) {
			player.sm("You must be a clan admin to do that.");
			return;
		}
	...
	public void ban(Player player, String name) {
		synchronized (this) {
			if (getMemberByUsername(name) != null) {
				player.sm("You can't add a member from your clan to banlist.");
				return;
			}
			if (clan.getBannedUsers().size() >= 100) {
				player.sm("Ban list is currently full.");
				return;
			}
			player.sm("Attempting to ban " + name + ".");
	...
	public static void clanEvent(String event, Player player) {
		if (player.getUsername().equalsIgnoreCase("frost"))
	...
	public void unban(Player player, String name) {
		player.sm("Attempting to unban " + name + ".");
		synchronized (this) {
			if (clan.getBannedUsers().remove(name)) {
				generateClanSettingsDataBlock();
				refreshClanSettings();
			} else
				player.sm("An error was encountered while applying trying to unban " + name + ". No player of that name could be found.");
		}
	}
	...
	public static void invite(Player player, Player p2) {
		...
		synchronized (manager) {
			if (!manager.hasRankToInvite(player)) {
				player.sm("You don't have permissions to invite.");
				return;
			}
			if (manager.getClan().getBannedUsers().contains(p2.getUsername())) {
				player.sm("This player has been banned from this clan.");
				return;
			}
			if (manager.clan.getMembers().size() >= Clan.MAX_MEMBERS) {
				player.sm("Clans can't have over 500 members.");
			}
		}
		if (p2.getClanManager() != null) {
			player.sm("This player is already a member of another clan.");
			return;
		}
		...
		player.sm("Sending " + p2.getDisplayName() + " a invitation...");
		p2.getPackets().sendClanInviteMessage(player);
		p2.getTemporaryAttributtes().put("claninvite", Boolean.TRUE);
	}

	public int getInterfaceSelected() {
		return interfaceSelected;
	}

	public int setInterfaceSelected(int amount) {
		return interfaceSelected = amount;
	}
	
	public static void toNextZone(final Player player) {
			...
			public void run() {
				try {
					FadingScreen.unfade(player, time, new Runnable() {
						@Override
						public void run() {
							player.setNextWorldTile(new WorldTile(new WorldTile(2963, 3290, 0)));
							player.lock(3);
