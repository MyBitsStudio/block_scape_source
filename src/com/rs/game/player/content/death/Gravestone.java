...
public class Gravestone extends NPC {
	...
	public static final void sendInterface(Player player) {
		...
		player.getPackets().sendConfig(1146, player.getPlayerVars().getGravestone() | 262112);
	}
	
	public static final void sendBuySelection(Player player, int option) {
		if (player.getPlayerVars().getGravestone() == option) {
		...
		player.getMoneyPouch().removeAmount(PRICES[option]);
		//player.getInventory().deleteItem(995, PRICES[option]); //doubles the deduction shit
		player.getPlayerVars().setGravestone(option);
	...
	public Gravestone(Player player, WorldTile deathTile, Item[] items) {
		super(getNPCId(player.getPlayerVars().getGravestone()), deathTile, -1, true, true);
		gravestone = player.getPlayerVars().getGravestone();
		...
		for (Item item : items) {
			if (item.getAmount() <= 0)
				continue;
			FloorItem i = World.addGroundItem(item, deathTile, player, true, -1, 0);
			if (i != null)
				floorItems.add(i);
		}
