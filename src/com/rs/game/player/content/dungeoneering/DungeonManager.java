...
public class DungeonManager {
	...
	public void clearKeyList() {
		for (Player player : party.getTeam()) {
			player.getPackets().sendRunScriptBlank(6072);
	...
	public void openRoom(final Room room, final RoomReference reference, final VisibleRoom visibleRoom) {
		...
		for (Player player : party.getTeam()) {
			...
			player.setNextWorldTile(player);
		}
		...
		if (party.getComplexity() >= 3 && Utils.random(3) == 0) { // sets rock
			for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				if (!World.isFloorFree(0, toChunkX * 8 + x, toChunkY * 8 + y))
					continue;
				World.spawnObject(
						new WorldObject(DungeonUtils.getMiningResource(Utils.random(DungeoneeringRocks.values().length),
								party.getFloorType()), 10, rotation, toChunkX * 8 + x, toChunkY * 8 + y, 0));
				if (Settings.DEBUG)
	...
	public void sendStartItems(Player player) {
		...
		if (party.getComplexity() <= 3) {
			...
			player.getInventory().addItem(
					new Item(DungeonUtils.getPlatelegs(defenceTier, player.getGlobalPlayerUpdater().isMale())));
			if (party.getComplexity() <= 2) {
	...
	public void sendSettings(Player player) {
		...
		if (player.getControlerManager().getControler() instanceof DungeonController)
			...
			player.getToolBeltNew().switchDungeonneringToolbelt();
			player.getMoneyPouch().closePouch();
	...
	public void spawnRandomNPCS(RoomReference reference) {
		...
		// hunter creature
		spawnNPC(reference, DungeonUtils.getHunterCreature(), 2 + Utils.getRandom(13), 2 + Utils.getRandom(13), true,
	...
	public void loadRewards() {
		...
		for (Player player : party.getTeam()) {
			...
			player.stopAll();
			double multiplier = 1;
			player.getInterfaceManager().sendFadingInterface(933);
			player.getPackets().sendRunScriptBlank(2275);
			for (int i = 0; i < 5; i++) {
				Player partyPlayer = i >= party.getTeam().size() ? null : party.getTeam().get(i);
				player.getPackets().sendGlobalConfig(1198 + i, partyPlayer != null ? 1 : 0);
				if (partyPlayer == null)
					continue;
			...
			player.getPackets().sendGlobalConfig(1195, v);
			multiplier += DungeonConstants.MAX_BONUS_ROOM * v / 10000;
			v = (getLevelModPerc() * 20) - 1000;
			player.getPackets().sendGlobalConfig(1236, v);
			multiplier += ((double) v) / 10000;
			player.getPackets().sendGlobalConfig(1196, party.isGuideMode() ? 1 : 0);
			if (party.isGuideMode())
				multiplier -= 0.05;
			player.getPackets().sendGlobalConfig(1319,
			...
			if (party.getTeam().size() == 1 && party.getSize() != DungeonConstants.LARGE_DUNGEON)
				averageXP *= 1.5;
			if (player.isBronze())
				averageXP *= player.isSilver() ? 1.15 : 1.1;
			...
			player.getSkills().addXp(Skills.DUNGEONEERING, totalXp, true);
	...
	public void openMap(Player player) {
		...
		player.getPackets().sendRunScriptBlank(3277); // clear the map if
														// theres any setted
