...
public class CrystalPuzzleRoom extends PuzzleRoom {
	...
	private static final int[][] TILE_ACTIVE = { { 49468, 49469, 49470, 54262, 34319 },
			{ 49477, 49478, 49479, 54265, 34848 }, { 49486, 49487, 49488, 54268, 34856 },
	...
	private static final int[][] LODESTONE_ACTIVE = { { 49474, 49475, 49476, 54264, 34847 },
			{ 49483, 49484, 49485, 54267, 34855 }, { 49492, 49493, 49494, 54270, 34861 },
			{ 49501, 49502, 49503, 54273, 34864 }, };

	// Inactive lodestone id's
	private static final int[][] LODESTONE_INACTIVE = { { 49471, 49472, 49473, 54263, 34320 },
			{ 49480, 49481, 49482, 54266, 34852 }, { 49489, 49490, 49491, 54269, 34860 },
	...
	public void openRoom() {
		...
		for (int color = 0; color < 4; color++) {
			if (Math.random() > 0.66 && activeCount++ != 3) {
				resetPosition(color);
				World.spawnObject(new WorldObject(LODESTONE_ACTIVE[color][type], 10, 0,
						manager.getTile(reference, POS_BASE_LODESTONE[0] + color, POS_BASE_LODESTONE[1] + color)));
			} else {
				World.spawnObject(new WorldObject(LODESTONE_INACTIVE[color][type], 10, 0,
						manager.getTile(reference, POS_BASE_LODESTONE[0] + color, POS_BASE_LODESTONE[1] + color)));
			}
		}
		task = new TileTask();
			...
			if (object.getId() == LODESTONE_INACTIVE[color][type]) {
				if (!hasRequirement(p, Skills.MAGIC)) {
					p.getPackets().sendGameMessage(
							"You need a magic level of " + getRequirement(Skills.MAGIC) + " to power this lodestone.");
					return false;
				}
	...
	private void resetPosition(int color) {
		...
		World.spawnObject(new WorldObject(TILE_ACTIVE[color][type], 22, 0,
				manager.getTile(reference, energyTile[color][0], energyTile[color][1])));
	}
		...
		public void run() {
			synchronized (manager) {
				...
				outer: for (int color = 0; color < 4; color++) {
					...
					// Remove current tile
					World.spawnObject(new WorldObject(TILE_INACTIVE[type], 22, 0,
							manager.getTile(reference, energyTile[color][0], energyTile[color][1])));
				...
				if (complete) {
					World.spawnObject(new WorldObject(LARGE_CRYSTAL_ON[type], 10, 0,
							manager.getTile(reference, POS_CENTER[0], POS_CENTER[1])));
					setComplete();
					stop();
					task = null;
				} else if (any) {
					World.spawnObject(new WorldObject(LARGE_CRYSTAL_FLASH[type], 10, 0,
							manager.getTile(reference, POS_CENTER[0], POS_CENTER[1])));
				} else {
					World.spawnObject(new WorldObject(LARGE_CRYSTAL_OFF[type], 10, 0,
							manager.getTile(reference, POS_CENTER[0], POS_CENTER[1])));
				}
			}
		}
