...
public class FishingFerretRoom extends PuzzleRoom {
	...
	public void openRoom() {
		...
		outer: for (int x = 0; x < 15; x++) {
			for (int y = 0; y < 15; y++) {
				...
				if (object != null && (object.getDefinitions().name.equals("Tile")
						|| object.getDefinitions().name.equals("Hole"))) {
		...
		pressurePlate = manager.getRotatedTile(reference, cornerChunks[invertChunks ? 2 : 0],
				cornerChunks[invertChunks ? 3 : 1]);
		Ferret puzzleNPC = new Ferret(FERRET_ID, manager.getRotatedTile(reference, cornerChunks[invertChunks ? 0 : 2],
				cornerChunks[invertChunks ? 1 : 3]), manager, 0.0);
		psuedoFishingSpot = new DungeonFishSpot(1957, manager.getRotatedTile(reference, 7, 13), manager,
				Fish.VILE_FISH);
		...
		if (object.getDefinitions().name.equals("Fishing spot")) {
			...
			if (psuedoFishingSpot == null)
				...
				player.getPackets().sendGameMessage(
						"You need a Fishing level of " + requiredFishing + " to catch a raw vile fish.");
				return false;
			}
			...
			if (player.getSkills().getLevel(Skills.COOKING) < requiredCooking) {
				player.getPackets().sendGameMessage(
						"You need a Cooking level of " + requiredCooking + " to cook a raw vile fish.");
				return false;
			}
	...
	public static boolean handleFerretThrow(final Player player, final WorldObject object, final Item item) {
		if (!object.getDefinitions().name.equals("Tile") && !object.getDefinitions().name.equals("Pressure plate")
				|| item.getId() != VILE_FISH || player.getControlerManager().getControler() == null
		...
		if (player.getSkills().getLevel(Skills.RANGE) < requiredRange) {
			player.getPackets()
					.sendGameMessage("You need a Range level of " + requiredRange + " to throw a vile fish.");
			return false;
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				puzzle.getVileFishes()
						.add(World.getRegion(player.getRegionId()).getGroundItem(item.getId(), object, player));
			}
		}, 2);
		return true;
	}
