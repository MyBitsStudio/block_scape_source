				...
				if (object.getId() == PLANTS[type][color][0] || object.getId() == PLANTS[type][color][1]) { // [1]
					...
					if (type == 0) {
						if (!hasRequirement(player, Skills.FARMING)) {
							player.getPackets().sendGameMessage("You need a farming level of "
									+ getRequirement(Skills.FARMING) + " to uproot this plant.");
							return false;
						}
						...
						if (!hasRequirement(player, Skills.WOODCUTTING)) {
							player.getPackets().sendGameMessage("You need a woodcutting level of "
									+ getRequirement(Skills.WOODCUTTING) + " to chop down this plant.");
							return false;
						}
						...
						player.applyHit(
								new Hit(player, (int) (player.getMaxHitpoints() * .15), HitLook.REGULAR_DAMAGE));
						return false;
					}
				}
			}
			...
			for (int x = 0; x < 16; x++) {
				for (int y = 0; y < 16; y++) {
					...
					if (p != null) {
						p.locked = true;
						if (p.type == 0) {
							team.getPackets().sendObjectAnimation(manager.getObjectWithType(reference, 10, x, y),
			...
			synchronized (manager) {
				...
				for (int x = 0; x < 16; x++) {
					for (int y = 0; y < 16; y++) {
						...
						if (p != null && !p.locked) {
							if (p.type == 0 || odd) {
								replaceObject(manager.getObjectWithType(reference, 10, x, y),
