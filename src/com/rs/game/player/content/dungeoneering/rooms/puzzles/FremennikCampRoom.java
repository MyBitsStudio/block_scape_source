		...
		if (object.getId() == RAW_FISH[type]) {
			if (!hasRequirement(player, Skills.COOKING)) {
				player.getPackets().sendGameMessage(
						"You need a cooking level of " + getRequirement(Skills.COOKING) + " to cook these fish.");
				return false;
			}
			...
			if (!hasRequirement(player, Skills.SMITHING)) {
				player.getPackets().sendGameMessage("You need a smithing level of " + getRequirement(Skills.SMITHING)
						+ " to smith these battle axes.");
				return false;
			}
			if (!player.getInventory().containsItem(DungeoneeringSmithing.DUNG_HAMMER, 1)) {
			...
			if (!hasRequirement(player, Skills.FLETCHING)) {
				player.getPackets().sendGameMessage(
						"You need a fletching level of " + getRequirement(Skills.FLETCHING) + " to fletch these bows.");
				return false;
			}
			if (!player.getInventory().containsItem(Fletching.DUNGEONEERING_KNIFE, 1)) {
