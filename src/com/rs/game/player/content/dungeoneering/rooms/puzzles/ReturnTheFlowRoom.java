			...
			while (node.next != null) {
				...
				if (!broken && !node.blocked) {
					manager.spawnObject(reference,
							node.corner ? CORNER_PIECE_PATH_FLOW[type] : STRAIGHT_PIECE_PATH_FLOW[type], 22,
							node.rotation, 5 + node.x, 6 + node.y);
				} else {
					manager.spawnObject(reference, node.corner ? CORNER_PIECE_PATH[type] : STRAIGHT_PIECE_PATH[type],
							22, node.rotation, 5 + node.x, 6 + node.y);
				}
				if (node.blocked) {
					if (!broken) {
						manager.spawnObject(reference, RUBBLE_PIECE_FLOW[type], 10, node.rotation, 5 + node.x,
								6 + node.y);
					} else {
	...
	public boolean processObjectClick1(final Player player, final WorldObject object) {
		if (object.getId() == RUBBLE_PIECE[type] || object.getId() == RUBBLE_PIECE_FLOW[type]) {
			if (!hasRequirement(player, Skills.MINING)) {
				player.getPackets().sendGameMessage(
						"You need a mining level of " + getRequirement(Skills.MINING) + " to min this rock.");
				return false;
			}
		...
		if (object.getId() == PILLAR_BROKEN[type]) {
			if (!hasRequirement(player, Skills.CONSTRUCTION)) {
				player.getPackets().sendGameMessage("You need a construction level of "
						+ getRequirement(Skills.CONSTRUCTION) + " to repair this pillar.");
				return false;
			}
	...
	private void startFlow(FlowPiece node) {
		while (node.next != null) {
			if (node.corner) {
				...
				manager.spawnObject(reference, STRAIGHT_PIECE_PATH_FLOW[type], 22, node.rotation, 5 + node.x,
						6 + node.y);
			} else {
