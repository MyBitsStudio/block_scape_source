...
public class ToxinMaze extends PuzzleRoom {
	...
	public boolean processObjectClick1(final Player player, WorldObject object) {
		...
		if (object.getId() == BARRIER_ENTRANCE && toxinTask == null && !isComplete()) {
			...
			WorldTile in = new WorldTile(object.getX() + Utils.ROTATION_DIR_X[object.getRotation()],
					object.getY() + Utils.ROTATION_DIR_Y[object.getRotation()], 0);
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					player.setNextForceMovement(
							new ForceMovement(fromTile, 0, target_, 1, getFaceDirection(target_, player)));
	...
	public void generate() {
		...
		for (Connector connector : connectors) {
			if (connector.blocked) {
				World.spawnObject(new WorldObject(OBSTACLES[type][Utils.random(5)], 10, 0,
						manager.getTile(reference, connector.x, connector.y)));
			}
		}
