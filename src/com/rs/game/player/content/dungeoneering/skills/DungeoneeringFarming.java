...
public class DungeoneeringFarming {
	...
	public static void initHarvest(final Player player, final Harvest harvest, final WorldObject object) {
		...
		if (isTextile) {
			if (player.getSkills().getLevel(Skills.FARMING) < harvest.lvl) {
				player.getPackets().sendGameMessage(
						"You need a Farming level of " + harvest.lvl + " in order to pick " + productName + ".");
				return;
			}
		}
		...
		if (harvestCount == 0) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					World.spawnObject(
							new WorldObject(isTextile ? object.getId() + 1 : DungeonConstants.EMPTY_FARMING_PATCH,
	...
	public static void plantHarvest(Item item, final Player player, WorldObject object, DungeonManager dungeon) {
		...
		if (harvest == null)
			return;
		else if (object.getDefinitions().containsOption("inspect") || object.getDefinitions().containsOption("clear")
				|| object.getDefinitions().containsOption("harvest")) {
			player.getPackets().sendGameMessage("This farming patch has something growing in it.");
			return;
		} else if (player.getSkills().getLevel(Skills.FARMING) < harvest.lvl) {
			player.getPackets().sendGameMessage("You need a Farming level of " + harvest.lvl + " in order to plant a "
					+ ItemDefinitions.getItemDefinitions(harvest.seed).getName().toLowerCase() + ".");
			return;
		}
		object.setId(DungeonConstants.EMPTY_FARMING_PATCH + 1);
		WorldObject[] objects = new WorldObject[4];
		for (int i = 0; i < objects.length; i++)
			objects[i] = new WorldObject(object.getId() + i + (harvest.ordinal() * 3), object.getType(),
					object.getRotation(), new WorldTile(object));
		dungeon.getFarmKeyList()
				.add(OwnedObjectManager.addOwnedObjectManager(player, objects, DungeonConstants.FARMING_CYCLES));
	...
	public static void clearHarvest(final Player player, final WorldObject object) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				World.spawnObject(new WorldObject(DungeonConstants.EMPTY_FARMING_PATCH, object.getType(),
