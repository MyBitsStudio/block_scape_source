...
public class DungeoneeringMining extends MiningBase {
	...
	private boolean checkAll(Player player) {
		if (axeDefinitions == null) {
			player.getPackets()
					.sendGameMessage("You do not have a pickaxe or do not have the required level to use the pickaxe.");
			return false;
		}
	...
	public int processWithDelay(Player player) {
		addOre(player);
		if (Utils.random(5) == 0) {
			World.spawnObject(new WorldObject(rock.getId() + 1, rock.getType(), rock.getRotation(), rock.getX(),
	...
	public static PickAxeDefinitions getPickAxeDefinitions(Player player, boolean dungeoneering) {
		for (int i = dungeoneering ? 10 : PickAxeDefinitions.values().length - 1; i >= (dungeoneering ? 0 : 11); i--) { // from
			...
			if (player.getInventory().containsItem(def.pickAxeId, 1)
