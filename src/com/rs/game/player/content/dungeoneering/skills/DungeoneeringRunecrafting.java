...
public class DungeoneeringRunecrafting extends Action {
	...
	public DungeoneeringRunecrafting(int cycles, int runeId, int levelRequirement, double experience,
			int... multipliers) {
		this.cycles = cycles;
		this.runeId = runeId;
	...
	public boolean start(Player player) {
		...
		if (actualLevel < levelRequirement) {
			player.getDialogueManager().startDialogue("SimpleMessage",
					"You need a runecrafting level of " + levelRequirement + " to craft this rune.");
			return false;
		}
		int essense = player.getInventory().getAmountOf(DungeonConstants.ESSENCE);
		if (essense == 0) {
			player.getPackets().sendGameMessage("You don't have enough rune essencse.");
			return false;
		}
		if (cycles < essense)
			cycles = essense;
