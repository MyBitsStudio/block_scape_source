...
public class DungeoneeringSmelting extends Action {
	...
	public DungeoneeringSmelting(SmeltingBar bar, WorldObject object, int ticks) {
		this.object = object;
		this.bar = bar;
		this.ticks = ticks;
	}
	...
	public boolean start(Player player) {
		...
		if (!player.getInventory().containsItemToolBelt(bar.getItemsRequired()[0].getId(),
		...
		if (bar.getItemsRequired().length > 1) {
			if (!player.getInventory().containsItemToolBelt(bar.getItemsRequired()[1].getId(),
	...
	public int processWithDelay(Player player) {
		...
		if (isSuccessFull(player)) {
			...
			player.getPackets()
					.sendGameMessage("You retrieve a bar of "
							+ bar.getProducedBar().getDefinitions().getName().toLowerCase().replace(" bar", "") + ".",
							true);
		} else {
