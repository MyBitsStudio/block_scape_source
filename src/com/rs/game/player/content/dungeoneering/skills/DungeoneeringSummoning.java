...
public class DungeoneeringSummoning {
	...
	public static Familiar createFamiliar(Player player, Pouch pouch) {
		try {
			return (Familiar) Class.forName("com.rs.game.npc.familiar."
					+ (NPCDefinitions.getNPCDefinitions(getNPCId(pouch.getRealPouchId()) + 1)).getName()
							.replace(" ", "").replace("�", "i").replace("-", "").replace("(", "").replace(")", ""))
	...
	public static enum Pouch {
		...
		LITTLE_BLOODRAGER(-1, 17936, 1, 19.5, 2700000, 2), LITTLE_DEATHSLINGER(-1, 17986, 1.5, 20.5, 2700000,
				2), LITTLE_STORMBRINGER(-1, 17946, 1.1, 21.5, 2700000, 2), LITTLE_HOARDSTALKER(-1, 17956, 1.2, 22.5,
						2700000, 2), LITTLE_WORLDBEARER(-1, 17976, 1.4, 23.5, 2700000,
								2), LITTLE_SKINWEAVER(-1, 17966, 1.3, 24.5, 2700000, 2),

		NAIVE_BLOODRAGER(-1, 17937, 1.5, 43, 2700000, 3), NAIVE_DEATHSLINGER(-1, 17987, 2, 44.4, 2700000,
				3), NAIVE_STORMBRINGER(-1, 17947, 1.6, 45.8, 2700000, 3), NAIVE_HOARDSTALKER(-1, 17957, 1.7, 47.2,
						2700000, 3), NAIVE_WORLDBEARER(-1, 17977, 1.9, 48.6, 2700000,
								3), NAIVE_SKINWEAVER(-1, 17967, 1.8, 50, 2700000, 3),

		KEEN_BLOODRAGER(-1, 17938, 2, 68.5, 2700000, 4), KEEN_DEATHSLINGER(-1, 17988, 2.5, 70.4, 2700000,
				4), KEEN_STORMBRINGER(-1, 17948, 2.1, 72.3, 2700000, 4), KEEN_HOARDSTALKER(-1, 17958, 2.2, 74.2,
						2700000, 4), KEEN_WORLDBEARER(-1, 17978, 2.4, 76.1, 2700000,
								4), KEEN_SKINWEAVER(-1, 17968, 2.3, 78, 2700000, 4),

		BRAVE_BLOODRAGER(-1, 17939, 2.5, 99.5, 2700000, 5), BRAVE_DEATHSLINGER(-1, 17989, 3, 102, 2700000,
				5), BRAVE_STORMBRINGER(-1, 17949, 2.6, 104.5, 2700000, 5), BRAVE_HOARDSTALKER(-1, 17959, 2.7, 107,
						2700000, 5), BRAVE_WORLDBEARER(-1, 17979, 2.9, 109.5, 2700000,
								5), BRAVE_SKINWEAVER(-1, 17969, 2.8, 112, 2700000, 5),

		BRAH_BLOODRAGER(-1, 17940, 3, 157, 2700000, 6), BRAH_DEATHSLINGER(-1, 17990, 3.5, 160.5, 2700000,
				6), BRAH_STORMBRINGER(-1, 17950, 3.1, 164, 2700000, 6), BRAH_HOARDSTALKER(-1, 17960, 3.2, 167.5,
						2700000, 6), BRAH_WORLDBEARER(-1, 17980, 3.4, 171, 2700000,
								6), BRAH_SKINWEAVER(-1, 17970, 3.3, 174.5, 2700000, 6),

		NAABE_BLOODRAGER(-1, 17941, 3.5, 220, 2700000, 7), NAABE_DEATHSLINGER(-1, 17991, 4, 224.6, 2700000,
				7), NAABE_STORMBRINGER(-1, 17951, 3.6, 229.2, 2700000, 7), NAABE_HOARDSTALKER(-1, 17961, 3.7, 233.8,
						2700000, 7), NAABE_WORLDBEARER(-1, 17981, 3.9, 238.4, 2700000,
								7), NAABE_SKINWEAVER(-1, 17971, 3.8, 243, 2700000, 7),

		WISE_BLOODRAGER(-1, 17942, 4, 325, 2700000, 8), WISE_DEATHSLINGER(-1, 17992, 4.5, 330.8, 2700000,
				8), WISE_STORMBRINGER(-1, 17952, 4.1, 336.6, 2700000, 8), WISE_HOARDSTALKER(-1, 17962, 4.2, 342.4,
						2700000, 8), WISE_WORLDBEARER(-1, 17982, 4.4, 348.2, 2700000,
								8), WISE_SKINWEAVER(-1, 17972, 4.3, 354, 2700000, 8),

		ADEPT_BLOODRAGER(-1, 17943, 4.5, 517.5, 2700000, 10), ADEPT_DEATHSLINGER(-1, 17993, 5, 524.6, 2700000,
				10), ADEPT_STORMBRINGER(-1, 17953, 4.6, 531.7, 2700000, 10), ADEPT_HOARDSTALKER(-1, 17963, 4.7, 538.8,
						2700000, 10), ADEPT_WORLDBEARER(-1, 17983, 4.9, 545.9, 2700000,
								10), ADEPT_SKINWEAVER(-1, 17973, 4.8, 553, 2700000, 10),

		SACHEM_BLOODRAGER(-1, 17944, 5, 810, 2700000, 10), SACHEM_DEATHSLINGER(-1, 17994, 5.5, 818.5, 2700000,
				10), SACHEM_STORMBRINGER(-1, 17954, 5.1, 827, 2700000, 10), SACHEM_HOARDSTALKER(-1, 17964, 5.2, 835.5,
						2700000, 10), SACHEM_WORLDBEARER(-1, 17984, 5.4, 844, 2700000,
								10), SACHEM_SKINWEAVER(-1, 17974, 5.3, 852.5, 2700000, 10),

		CLAY_BEAST1(-1, 14422, 0, 0, 1800000, 1), CLAY_BEAST2(-1, 14424, 0, 0, 1800000, 3), CLAY_BEAST3(-1, 14426, 0, 0,
				1800000, 5), CLAY_BEAST4(-1, 14428, 0, 0, 1800000, 7), CLAY_BEAST5(-1, 14430, 0, 0, 1800000, 10);
	...
	public static void openInfusionInterface(Player player, boolean is_scroll, boolean dungeoneering) {
		player.getVarsManager().sendVarBit(8701, player.isPouchFilter() ? 1 : 0);
		if (is_scroll)
			player.getPackets().sendScrollInfusionOptionsScript(dungeoneering, POUCHES_INTERFACE, 16, 78, 7, 10,
					"Transform<col=FF9040>", "Transform-5<col=FF9040>", "Transform-10<col=FF9040>",
					"Transform-X<col=FF9040>", "Transform-All<col=FF9040>");
		else
			player.getPackets().sendPouchInfusionOptionsScript(dungeoneering, POUCHES_INTERFACE, 16, 78, 7, 10,
					"Infuse<col=FF9040>", "Infuse-5<col=FF9040>", "Infuse-10<col=FF9040>", "Infuse-X<col=FF9040>",
					"Infuse-All<col=FF9040>", "List<col=FF9040>");
		player.getPackets().sendHideIComponent(79, 23, !is_scroll);
		player.getPackets().sendHideIComponent(79, 26, !is_scroll);
		player.getPackets().sendHideIComponent(79, 27, !is_scroll);
	...
	public static void handlePouchInfusion(Player player, int slotId, int itemId, int creationCount,
			boolean dungeoneering) {
		Pouch pouch = player.isPouchFilter() ? Pouch.forId(itemId)
				: Pouch.values()[(dungeoneering ? 78 : 0) + (slotId - 2) / 5];
		if (pouch == null)
			return;
	...
	public static void sendItemList(Player player, boolean infusingScroll, int count, int slotId, int itemId,
			boolean dungeoneering) {
		Pouch pouch = player.isPouchFilter() ? Pouch.forId(itemId)
				: Pouch.values()[(dungeoneering ? 78 : 0) + (slotId - 2) / 5];
		if (pouch == null)
	...
	public static void handleInfusionOptions(Player player, int packetId, int componentId, int slotId, int slotId2,
			boolean dungeoneering) {
		if (componentId == 16) {
			if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
				DungeoneeringSummoning.handlePouchInfusion(player, slotId, slotId2, 1, dungeoneering);
			else if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET)
				DungeoneeringSummoning.handlePouchInfusion(player, slotId, slotId2, 5, dungeoneering);
			else if (packetId == WorldPacketsDecoder.ACTION_BUTTON3_PACKET)
				DungeoneeringSummoning.handlePouchInfusion(player, slotId, slotId2, 10, dungeoneering);
			else if (packetId == WorldPacketsDecoder.ACTION_BUTTON4_PACKET) {
				player.getPackets().sendInputIntegerScript(true, "Select an amount to infuse:");
				...
				DungeoneeringSummoning.handlePouchInfusion(player, slotId, slotId2, 28, dungeoneering);
			else if (packetId == WorldPacketsDecoder.ACTION_BUTTON6_PACKET) {
				DungeoneeringSummoning.sendItemList(player,
						(boolean) player.getTemporaryAttributtes().get("infusing_scroll"), 1, slotId, slotId2,
						dungeoneering);
			}
		} else if (componentId == 22 || componentId == 20) {
			player.closeInterfaces();
			DungeoneeringSummoning.openInfusionInterface(player, componentId == 20, dungeoneering);
		} else if (componentId == 29) {
			player.setPouchFilter(!player.isPouchFilter());
			player.closeInterfaces();
			DungeoneeringSummoning.openInfusionInterface(player,
					(boolean) player.getTemporaryAttributtes().get("infusing_scroll"), dungeoneering);
		}
	}
}
