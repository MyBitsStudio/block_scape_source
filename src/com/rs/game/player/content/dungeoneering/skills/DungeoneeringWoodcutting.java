...
public final class DungeoneeringWoodcutting extends Action {
	...
	public static enum TreeDefinitions {
		...
		private TreeDefinitions(int level, double xp, int logsId, int logBaseTime, int logRandomTime, int stumpId,
				int respawnDelay, int randomLifeProbability) {
			this(level, xp, new int[] { logsId }, logBaseTime, logRandomTime, stumpId, respawnDelay,
					randomLifeProbability);
		}
	...
	public DungeoneeringWoodcutting(WorldObject tree, TreeDefinitions definitions, boolean usingBeaver) {
		this.tree = tree;
		this.definitions = definitions;
		this.usingBeaver = usingBeaver;
	}

	public DungeoneeringWoodcutting(WorldObject tree, TreeDefinitions definitions) {
		this(tree, definitions, false);
	}

	public DungeoneeringWoodcutting(ElderTree tree) {
		this.elder = tree;
	...
	public boolean start(Player player) {
		if (!checkAll(player))
			return false;
		player.getPackets().sendGameMessage(usingBeaver
				? "Your beaver uses its strong ivory teeth to chop down the tree..."
	...
	private boolean checkAll(Player player) {
		hatchet = getHatchet(player, definitions.ordinal() > 19);
		if (hatchet == null) {
			player.getPackets()
					.sendGameMessage("You dont have the required level to use that axe or you don't have a hatchet.");
			return false;
		}
	...
	public int processWithDelay(Player player) {
		boolean dungeoneering = definitions.ordinal() > 19;
		...
		if (elder != null) {
			...
			if (time < 0) {
				...
				World.spawnObjectTemporary(new WorldObject(dungeoneering ? tree.getId() + 1 : definitions.getStumpId(),
						tree.getType(), tree.getRotation(), tree.getX(), tree.getY(), tree.getPlane()), time, false,
						true);
