		...
		if (slotId == Equipment.SLOT_RING) {
			...
			if (choosenOption == 0) {
				for (int i = 0; i <= Cosmetics.MENAPHITE_ANCIENT_OUTFIT.ordinal(); i++) {
					...
					if ((keyWord != null && !cos.getName().toLowerCase().contains(keyWord)) || (player.getPlayerVars().isFilterLocked() && CosmeticsHandler.isRestrictedItem(player, cos.getCosmetics())))
						continue;
			...
			for (Cosmetics cos : Cosmetics.values()) {
				...
				for (Cosmetic cosmetic : cos.getCosmetics()) {
					if (cosmetic.getSlot() != slotId || (keyWord != null && !cosmetic.getName().toLowerCase().contains(keyWord)) || (player.getPlayerVars().isFilterLocked() && CosmeticsHandler.isRestrictedItem(player, cosmetic)))
						continue;
					availableCostumes.add(new Object[] { cosmetic.getName(), new Cosmetic[] { cosmetic } });
				}
			}
		}
		if ((keyWord != null || player.getPlayerVars().isFilterLocked()) && availableCostumes.isEmpty()) {
			player.getDialogueManager().startDialogue("SimpleMessage", player.getPlayerVars().isFilterLocked() ? "You don't have any unlocked cosmetic in this slot." : "We couldn't find any available costume with keyword:" + keyWord + " .");
			return;
		}
		currentPage = 0;
