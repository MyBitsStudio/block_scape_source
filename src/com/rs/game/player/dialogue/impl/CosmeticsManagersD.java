...
public class CosmeticsManagersD extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		...
		case 2:
			if (componentId == OPTION_1) {
				...
				player.getPlayerVars().setFilterLocked(!player.getPlayerVars().isFilterLocked());
				player.getPackets().sendGameMessage("You will now see " + (!player.getPlayerVars().isFilterLocked() ? "all the cosmetics" : "only the cosmetics u have unlocked") + ".");
			} else if (componentId == OPTION_2) {
				end();
