...
public class FremennikShipmaster extends Dialogue {
    	...
    	if (backing) {
    		if (stage == -1) {
    			...
    			sendOptionsDialogue(SEND_DEFAULT_OPTIONS_TITLE, 
    					"Yes, please.", "Not right now, thanks.", "You look happy.");
    		} 
    		else if (stage == 0) {
    		...
    		if (stage == -1) {
    			...
    			stage = 2;
    			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[] { 
    					NPCDefinitions.getNPCDefinitions(npcId).name, "Well, don't stand arround. Get on board." }, 
    					IS_NPC, npcId, 9827);
    		} else if (stage == 2) {
    			sail(player, backing);
    			end();
    ...
    public void start() {
    	...
    	if (backing) {
    		sendEntityDialogue(SEND_1_TEXT_CHAT, new String[] { 
    				NPCDefinitions.getNPCDefinitions(npcId).name, "Do you want a lift back to the south?" }, 
    				IS_NPC, npcId, 9827);
    	} else {
    		sendEntityDialogue(SEND_1_TEXT_CHAT, new String[] { 
    				NPCDefinitions.getNPCDefinitions(npcId).name, "You want passage to Daemonheim?" }, 
    				IS_NPC, npcId, 9827);
    	}
    }
}