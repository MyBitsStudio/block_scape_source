	...
	public void run(int interfaceId, int componentId) {
		...
		case -1:
			sendNPCDialogue(npcId, CALM, "Certainly. That'll cost you " +
					((player.getPerks().isUnlocked(Perks.PERSLAY.getId()) || Settings.DEBUG) ? 0 : 10)
					+" Slayer points.");
			stage = 0;
			break;
		case 0:
		...
		case 1:
			...
			break;
		case 2:
			if (player.getSlayerPoints() < ((player.getPerks().isUnlocked(Perks.PERSLAY.getId())|| Settings.DEBUG) ? 0 : 10)) {
			...
			player.setTask(null);
			player.setSlayerPoints(player.getSlayerPoints() - ((player.getPerks().isUnlocked(Perks.PERSLAY.getId())|| Settings.DEBUG) ? 0 : 10));
