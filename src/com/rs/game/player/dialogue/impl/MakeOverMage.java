	...
	if (stage == -1) {
	    ...
	    if (componentId == OPTION_1) {
		    ...
		    sendEntityDialogue(SEND_1_TEXT_CHAT, new String[] { 
		    		player.getDisplayName(), "Sure, here you go." }, IS_PLAYER, player.getIndex(), 9827);
		
	    } else {
	    	stage = -2;
	    	sendEntityDialogue(SEND_1_TEXT_CHAT, new String[] { 
	    			player.getDisplayName(), "No way! That's too expensive." }, IS_PLAYER,
			player.getIndex(), 9827);
	    }
	} else if (stage == 17) {
	    stage = 18;
	    ...
	    if (player.getInventory().hasFreeSlots()) {
	    	...
	    	World.updateGroundItem(new Item(7803), player, player, 60, 0);
		    sendItemDialogue(7803, 1, "You receive an amulet, which was dropped under you, in exchange for 100 coins.");
	    }
	} else if (stage == 18) {
	    stage = 0;
	    sendEntityDialogue(
