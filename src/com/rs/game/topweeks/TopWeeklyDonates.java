...
public class TopWeeklyDonates {
	...
	public static void openInterface(Player player) {
		...
		for(int i=InterfaceManager.INTER_275_START_BODY;i<=InterfaceManager.INTER_275_END_BODY;i++) {
			...
			player.getPackets().sendIComponentText(275, i, (index == 0 ? Colors.green : index == 1 ? Colors.orange : index == 2 ? Colors.blue : "")+""+Utils.formatString(((Map.Entry<String, Double>) vote).getKey())+" - $"+((Map.Entry<String, Double>) vote).getValue());
			index++;
		}
	}
