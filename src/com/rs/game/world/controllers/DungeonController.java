...
import com.rs.game.player.content.dungeoneering.skills.DungeoneeringConsumables;
...
import com.rs.game.player.content.dungeoneering.skills.DungeoneeringMining.PickAxeDefinitions;
...
import com.rs.game.player.content.dungeoneering.skills.DungeoneeringSummoning.Pouch;
import com.rs.game.player.content.dungeoneering.skills.DungeoneeringTraps;
import com.rs.game.player.content.dungeoneering.skills.DungeoneeringWoodcutting;
import com.rs.game.player.content.dungeoneering.skills.DungeoneeringWoodcutting.HatchetDefinitions;
import com.rs.game.player.content.dungeoneering.skills.DungeoneeringWoodcutting.TreeDefinitions;
import com.rs.game.player.dialogue.Dialogue;
import com.rs.game.player.dialogue.impl.LeatherCraftingD;
...
public class DungeonController extends Controller {
		...
		if (room != null) {
			if (room.getRoom() == DungeonUtils.getBossRoomWithChunk(DungeonConstants.FROZEN_FLOORS, 26, 624)) {
				if (!player.isCantWalk()
					...
					player.getGlobalPlayerUpdater().setRenderEmote(1429);
					player.setRun(true);
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		...
		if (player.getDungManager().isInside()) {
			player.getPackets().sendGameMessage("You can't teleport out before leaving the dungeon.");
			return false;
		}
		return true;
	}
	...
	private void openSkillDoor(final SkillDoors s, final WorldObject object, final Room room, final int floorType) {
		...
		if (s.getSkillId() == Skills.FIREMAKING) {
			...
			PickAxeDefinitions defs = DungeoneeringMining.getPickAxeDefinitions(player, true);
			if (defs == null) {
			...
			HatchetDefinitions defs = DungeoneeringWoodcutting.getHatchet(player, true);
			if (defs == null) {
				player.getPackets().sendGameMessage(
	...
	public boolean processNPCClick1(final NPC npc) {
		...
		if (npc.getId() == DungeonConstants.FISH_SPOT_NPC_ID) {
			...
			adventurer.getFrozenPlayer().getGlobalPlayerUpdater().transformIntoNPC(-1);
			return false;
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		case "tangle gum tree":
			player.getActionManager().setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.TANGLE_GUM_VINE));
			return false;
		case "seeping elm tree":
			player.getActionManager().setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.SEEPING_ELM_TREE));
			return false;
		case "blood spindle tree":
			player.getActionManager()
					.setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.BLOOD_SPINDLE_TREE));
			return false;
		case "utuku tree":
			player.getActionManager().setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.UTUKU_TREE));
			return false;
		case "spinebeam tree":
			player.getActionManager().setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.SPINEBEAM_TREE));
			return false;
		case "bovistrangler tree":
			player.getActionManager()
					.setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.BOVISTRANGLER_TREE));
			return false;
		case "thigat tree":
			player.getActionManager().setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.THIGAT_TREE));
			return false;
		case "corpsethorn tree":
			player.getActionManager().setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.CORPESTHORN_TREE));
			return false;
		case "entgallow tree":
			player.getActionManager().setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.ENTGALLOW_TREE));
			return false;
		case "grave creeper tree":
			player.getActionManager()
					.setAction(new DungeoneeringWoodcutting(object, TreeDefinitions.GRAVE_CREEPER_TREE));
			return false;
		case "novite ore":
	...
	public boolean processObjectClick2(final WorldObject object) {
		...
		String name = object.getDefinitions().name.toLowerCase();
		if ("runecrafting altar".equals(name)) {
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		...
		case "farming patch":
			...
			return true;
		case "spinning wheel":
			player.getDialogueManager().startDialogue("SpinningD", true, item);
			return true;
		case "furnace":
	...
	public boolean canUseItemOnItem(Item itemUsed, Item usedWith) {
		...
		if (itemUsed.getId() == LeatherCraftingD.DUNG_NEEDLE || usedWith.getId() == LeatherCraftingD.DUNG_NEEDLE) {
			...
			Fletch fletch = DungeoneeringFletching.isFletching(itemUsed, usedWith);
			if (fletch != null) {
				player.getDialogueManager().startDialogue("DungeoneeringFletchingD", fletch);
				return false;
			}
		}
		return true;
	...
	public void openDoor(WorldObject object) {
		...
		dungeon.enterRoom(player, roomReference.getX() + Utils.ROTATION_DIR_X[object.getRotation()],
				roomReference.getY() + Utils.ROTATION_DIR_Y[object.getRotation()]);
		hideBar();
	}
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		...
		if (dungeon.isAtRewardsScreen()) {
			...
			if (interfaceId == 548 || interfaceId == 746) {
				...
				if ((interfaceId == 548 && componentId == 159) || (interfaceId == 746 && componentId == 207)) {
					player.getPackets().sendGameMessage("You can't do that in here.");
					return false;
				}
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					if (DungeoneeringConsumables.eat(player, slotId, item))
						return false;
					...
					if (pouch != null) {
						DungeoneeringSummoning.spawnFamiliar(player, pouch);
						return false;
					}
				...
				if (componentId == 2)
					...
					player.getActionManager().setAction(new HomeTeleport(dungeon.getHomeTile(), true));
					...
					Magic.processDungSpell(player, componentId, slotId, packetId);
				return false;
	...
	private void stoneTeleport(boolean group) {
		if (!player.getInventory().containsItem(DungeonConstants.RUNES[12], 3)) {
			player.getPackets().sendGameMessage("You do not have enough "
					+ ItemDefinitions.getItemDefinitions(DungeonConstants.RUNES[12]).getName().replace("rune", "Rune")
		...
		if (dungeon.isAtBossRoom(player, 26, 626, true)
			...
			return;
		} else if (dungeon == null || !player.getCombatDefinitions().isDungeonneringSpellBook() || !dungeon.hasStarted()
				|| dungeon.isAtRewardsScreen())
			return;
		...
		Magic.sendTeleportSpell(player, 13288, 13285, 2516, 2517, group ? 64 : 32, 0, tile, 3, false,
				Magic.MAGIC_TELEPORT, true);
		player.getInventory().deleteItem(DungeonConstants.RUNES[12], 3);
		if (!group) {
	...
	private void portalGroupStoneTeleport() {
		...
		if (dungeon == null || !player.getCombatDefinitions().isDungeonneringSpellBook() || !dungeon.hasStarted()
				|| dungeon.isAtRewardsScreen())
			return;
		Magic.sendTeleportSpell(player, 14279, 13285, 2518, 2517, 1, 0, tile, 1, false, Magic.OBJECT_TELEPORT, true);
	}
	...
	private boolean canCreateGatestone() {
		if (!Magic.checkSpellLevel(player, 32))
			...
			player.getPackets().sendGameMessage("You do not have enough "
					+ ItemDefinitions.getItemDefinitions(Magic.DUNG_COSMIC_RUNE).getName().replace("rune", "Rune")
					+ "s to cast this spell.");
			return false;
		}
		return true;
	...
	public boolean processItemOnPlayer(Player p2, Item item, int slot) {
		if (DungeoneeringConsumables.isConsumable(item))
			DungeoneeringConsumables.eat(p2, slot, item);
		return true;
	}
