...
public class Dungeoneering extends Controller {
	...
	public static boolean DragonKinArea(Player player) {
	...
	private int getKcRequired() {
		if (player.getPerks().isUnlocked(Perks.DUNGEON.getId()))
			return 5;
		return 15;
	}
	...
	private void handleDoor2(int destX, int destY, boolean requireKC) {
		if (requireKC) {
			if (player.getSkills().getLevel(Skills.DUNGEONEERING) < 90) {
				player.sendMessage("You'll need a Dungeoneering level of " + 90 + " to enter this room.");
				return;
			}
	...
	private void handleDoor3(int destX, int destY, boolean requireKC) {
		if (requireKC) {
			if (player.getSkills().getLevel(Skills.DUNGEONEERING) < 90) {
				player.sendMessage("You'll need a Dungeoneering level of " + 90 + " to enter this room.");
				return;
	...
	private void handleDoor4(int destX, int destY, boolean requireKC) {
		if (requireKC) {
			if (player.getSkills().getLevel(Skills.DUNGEONEERING) < 90) {
				player.sendMessage("You'll need a Dungeoneering level of " + 90 + " to enter this room.");
				return;
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 11005) {
			...
			if (object.getX() == 5028 && object.getY() == 9219) {
				if (player.getY() >= 9220) {
					handleDoor2(5028, 9218, true);
					player.StarfireBoss1 = true;
				} else {
					handleDoor2(5028, 9220, false);
				}
				return false;
			}
			...
			if (object.getX() == 5040 && object.getY() == 9199) {
				if (player.getY() >= 9200)
					handleDoor2(5040, 9180, true);
				else
					handleDoor2(5040, 9200, false);
				return false;
			}

			if (object.getX() == 5040 && object.getY() == 9181) {
				if (player.getY() >= 9182)
					handleDoor3(5040, 9178, true);
				else
					handleDoor3(5040, 9182, false);
				return false;
			}
			...
			if (object.getX() == 5010 && object.getY() == 9163) {
				if (player.getY() >= 9162)
					handleDoor2(5010, 9164, true);
				else
					handleDoor2(5010, 9162, false);
				return false;
			}
			if (object.getX() == 4998 && object.getY() == 9151) {
				if (player.getX() >= 4999)
					handleDoor2(4997, 9151, true);
				else
					handleDoor2(4999, 9151, false);
				return false;
			}
			// getinto elegorn
			...
			if (object.getX() == 5003 && object.getY() == 9126) {
				if (player.getY() >= 9127)
					handleDoor4(5003, 9125, true);
				else
					handleDoor4(5003, 9127, false);
				return false;
			}
			...
			if (object.getX() == 5048 && object.getY() == 9079) {
				if (player.getX() >= 5049 || player.getX() >= 5048)
					handleDoor4(5047, 9078, true);
				else
					handleDoor4(5048, 9079, false);
				return false;
			}
			if (object.getX() == 5072 && object.getY() == 9069) {
				if (player.getX() >= 5071)
					handleDoor4(5073, 9069, true);
				else
					handleDoor4(5071, 9069, false);
				return false;
			}
			...
			if (object.getX() == 5051 && object.getY() == 9032) {
				if (player.getX() >= 5050)
					handleDoor4(5053, 9032, true);
				else
					handleDoor4(5050, 9032, false);
	...
	private static void handleReward(Player player, int xp, int tokens) {
		...
		player.setDungeoneeringTokens((int) (player.getDungeoneeringTokens()
				+ ((player.getPerks().isUnlocked(Perks.DUNGEON.getId()) ? 1.25 : 1) * tokens * (WorldSettings.isDungeonWeekend() ? 2 : 1))));
		player.sm("Dungeoneering tokens received: " + Colors.red
				+ ((player.getPerks().isUnlocked(Perks.DUNGEON.getId()) ? 1.25 : 1) * tokens * (WorldSettings.isDungeonWeekend() ? 2 : 1)));
		player.getSkills().addXp(Skills.DUNGEONEERING,
				(player.getPerks().isUnlocked(Perks.DUNGEON.getId()) ? 1.25 : 1) * xp * (WorldSettings.isDungeonWeekend() ? 2 : 1));
