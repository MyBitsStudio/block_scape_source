...
public class FightKiln extends Controller {
    ...
    public static void enterFightKiln(Player player, boolean quickEnter) {
		...
		if (familiar != null && ((familiar.getBob() != null && 
				familiar.getBob().containsOneItem(23653, 23654, 23655, 23656, 23657,23658)) || familiar.isFinished()))
		    return;
		if (!quickEnter)
		    ...
		    if (type == 1 || type == 4) {
				...
				if (type == 4) {
				    World.sendWorldMessage("<img=7><col=CC0000>Server: " + player.getDisplayName() + " "
				    		+ "has just completed the Fight Kiln minigame.", false);

					/*new Thread(new NewsManager(player, "<b><img src=\"./bin/images/news/fight_kiln.png\" height=17> "
							+ player.getDisplayName()+" has just completed the Fight Kiln minigame.")).start();*/
					
				    Item reward = (Item) player.getTemporaryAttributtes().get("FightKilnReward");
	...
	Runnable event = new Runnable() {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			if (boundChuncks == null) {
			    boundChuncks = MapBuilder.findEmptyChunkBound(8,
				    8);
			    buildMap();
			} else if (!login
    ...
    public void nextWave() {
		...
		if (logoutAtEnd) {
			...
			|| nextWave == 31 || nextWave == 34 || nextWave == 37 || nextWave == 38) {
		    harAken = null;
		    player.stopAll();
		    loadCave(false);
		    return;
    ...
    public void startWave() {
		if (stage != Stages.RUNNING)
		    return;
		int currentWave = getCurrentWave();
		player.getInterfaceManager().sendTab(player.getInterfaceManager().hasRezizableScreen() ? 69 : 47, 316);
