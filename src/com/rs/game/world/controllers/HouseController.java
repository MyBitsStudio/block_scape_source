...
public class HouseController extends Controller {
	...
	public boolean processNPCClick1(NPC npc) {
		 ...
		 * npc.getId(), "Sorry, I only serve my master."); return false; }
	...
	public boolean processNPCClick2(NPC npc) {
		 ...
		 * (!house.isOwner(player)) {
		 * player.getDialogueManager().startDialogue("SimpleNPCMessage",
		 * npc.getId(), "The servant ignores your request."); return false; }
	...
	public boolean processObjectClick1(WorldObject object) {
		if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
			...
			player.sendMessage("Use some logs on the fireplace in order to light it.");
				...
				|| object.getId() == HouseConstants.HObject.WOODEN_LARDER.getId()) {
			...
			return false;
			// } else if
			// (HouseConstants.Builds.TELESCOPE.containsObject(object)) {
			...
			if (house.isBuildMode()) {
				...
				return false;
					...
					|| object.getId() >= HouseConstants.HObject.INSCENCE_BURNER.getId()
			...
			if (!house.isBuildMode()) {
				byte ringType = (byte) (this.ringType > 0 ? -1
						: (object.getId() == 13137 ? 3 : object.getId() == 13133 ? 2 : 1));
				if (!canEnterRing(ringType))
					return false;
				this.ringType = ringType;
			}
			if (object.getRotation() == 0)
				player.useStairs(4853, new WorldTile(player.getX() == object.getX() ? object.getX() - 1 : object.getX(),
						object.getY(), object.getPlane()), 2, 3);
			else if (object.getRotation() == 2)
				player.useStairs(4853, new WorldTile(player.getX() == object.getX() ? object.getX() + 1 : object.getX(),
						object.getY(), object.getPlane()), 2, 3);
			else if (object.getRotation() == 1)
				player.useStairs(
						4853, new WorldTile(object.getX(),
								player.getY() == object.getY() ? object.getY() + 1 : object.getY(), object.getPlane()),
						2, 3);
			else
				player.useStairs(
						4853, new WorldTile(object.getX(),
			...
			Magic.sendTeleportSpell(player, -1, -1, -1, -1, 0, 0,
					HouseConstants.PORTAL_COORDINATES[object.getId() - (13615 + 7 * (((object.getId() - 13615) / 7)))],
					0, true, Magic.OBJECT_TELEPORT);
			return false;
		} else if (object.getDefinitions().containsOption(0, "Build")) {
			if (!house.isOwner(player)) {
				player.sendMessage("You can only do that in your own house.");
				return false;
			}
	...
	/**
	 * @Override public boolean canRemoveEquip(int slot, int itemId) { if
	 *           (ringType == 1) { if (slot == Equipment.SLOT_WEAPON) {
	 *           player.sendMessage("You can't remove your boxing gloves.");
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		if (object.getId() == HouseConstants.HObject.CLAY_FIREPLACE.getId()
			...
			player.getDialogueManager().startDialogue("SimpleMessage", "You can't cook that on a "
	...
	public boolean processObjectClick2(final WorldObject object) {
		if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				@Override
				public void run() {
					if (Utils.random(player.getSkills().getLevel(Skills.THIEVING)) < Utils
	...
	public boolean processObjectClick3(final WorldObject object) {
		if (HouseConstants.Builds.STAIRCASE.containsObject(object)
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				@Override
				public void run() {
					if (Utils.random(player.getSkills().getLevel(Skills.STRENGTH)) < Utils
							.random(HouseConstants.DUNGEON_DOOR_LEVELS[slot])) {
	...
	public boolean processObjectClick4(WorldObject object) {
		if (HouseConstants.Builds.LADDER.containsObject(object) || HouseConstants.Builds.TRAPDOOR.containsObject(object)
				|| object.getId() == HouseConstants.HObject.DUNGEON_ENTRACE.getId()
				|| HouseConstants.Builds.STAIRCASE.containsObject(object)
				|| HouseConstants.Builds.STAIRCASE_DOWN.containsObject(object)) {
			if (!house.isOwner(player)) {
	...
	public boolean canMove(final int dir) {
		...
		final WorldObject trap = house.getDungeonTrap(new WorldTile(player.getX() + Utils.DIRECTION_DELTA_X[dir],
							...
							if (slot == 0 || slot == 1) {
								...
								if (obliette != null) {
									...
									if (isPool) {
										...
										player.getGlobalPlayerUpdater().setRenderEmote(152);
									}
									player.useStairs(isPool ? 3641 : 3640,
											Utils.getFreeTile(house.getCenterTile(obliette), 1), 0, 2, null, true);
								}
							}
						}
					} else {
						if (count == 3) {
							player.setNextForceMovement(new NewForceMovement(player, 0, successTile, 1, Utils
									.getAngle(trap.getX() - successTile.getX(), trap.getY() - successTile.getY())));
