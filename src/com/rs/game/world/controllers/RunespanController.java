...
public class RunespanController extends Controller {
		...
		private static Object[] getToPlataform(WorldTile fromPlataform) {
			for (HandledPlatforms toPlatraform : HandledPlatforms.values()) {
				if (toPlatraform.smallIsland.matches(fromPlataform))
					return new Object[]
					{ toPlatraform.largeIsland, true };
				else if (toPlatraform.largeIsland.matches(fromPlataform))
					return new Object[]
					{ toPlatraform.smallIsland, false };
			}
			return null;
	...
	private static int AIR_RUNE = 24215, EARTH_RUNE = 24216, WATER_RUNE = 24214, MIND_RUNE = 24217, FIRE_RUNE = 24213, ELEMENTAL_RUNE = -1, BODY_RUNE = 24218, RUNE_ESSENCE = 24227,
			CHAOS_RUNE = 24221, NATURE_RUNE = 24220, COSMIC_RUNE = 24223, ASTRAL_RUNE = 24224, LAW_RUNE = 24222, BLOOD_RUNE = 24225, DEATH_RUNE = 24219;
	...
	public static void enterRunespan(final Player player) {
		player.useStairs(-1, LOWER_LEVEL, 0, 2);
			...
			for (int runeId : plataform.runes) {
				if (runeId == ELEMENTAL_RUNE) {
					if (player.getInventory().containsOneItem(AIR_RUNE))
						...
						&& !player.getUsername().equalsIgnoreCase("Zeus")
						&& !RuneCrafting.hasEtherealOutfit(player)) {
			...
			public void run() {
				if (stage == 0) {
					...
					if (plataform.invisible)
						player.getGlobalPlayerUpdater().transformIntoNPC(1957);
				} else if (stage == 5) {
					if (plataform.invisible)
	...
	public boolean processNPCClick3(NPC npc) {
		if (npc.getId() == 15418) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", 15418, "I don't sell any runes.");
			return false;
		}
