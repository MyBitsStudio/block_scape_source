	...
	public static final void loadItemSpawns(int regionId) {
		...
		try {
			...
			while (buffer.hasRemaining()) {
				...
				World.addGroundItemForever(
						new Item(itemId, ItemDefinitions.getItemDefinitions(itemId).isStackable() ? 2 : 1),
						new WorldTile(x, y, plane));
			}
			channel.close();
			in.close();
	...
	private static final void addItemSpawn(int itemId, int regionId, WorldTile tile) {
		try {
			DataOutputStream out = new DataOutputStream(
					new FileOutputStream("data/items/packedSpawns/" + regionId + ".is", true));
